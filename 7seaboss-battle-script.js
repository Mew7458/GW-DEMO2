...
function createUnit(id, name, side, level, r, c, maxHp, maxSp, restoreOnZeroPct, spZeroHpPenalty=0, passives=[], extra={}){
  return {
    ...
    status: {
      ...
      paralyzed: 0,
      bleed: 0,
      bleedStrength: 0,        // 普通流血总强度（每点强度=2HP）
      bleedStrengthQueue: [],  // 每层流血对应的强度队列（每回合消耗一层）
      hazBleedTurns: 0,
  ...
  if(u.id==='adora'){
    ...
    if(!u._comeback){
      F.push(
        ...
        { key:'猎神之叉', prob:0.65, cond:()=>true, make:()=> skill('猎神之叉',2,'red','5x5内选择敌人：瞬移至其身旁并造成20(50%概率x2)+15SP并施加1层流血（3层强度）',
