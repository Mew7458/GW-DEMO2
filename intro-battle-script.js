...
function _audioPath(filename){
  // Spaces and non-ascii need URL encoding for reliable loading
  return encodeURI(`SFX/${filename}`);
}
function preloadSfx(){
  try{
    const files = [
    ...
    for(const fn of files){
      const a = new Audio(_audioPath(fn));
      a.preload = 'auto';
      a.volume = 0.92;
      SFX[fn] = a;
    }
    // Slight per-file tuning (optional)
    if(SFX["Death.mp3"]) SFX["Death.mp3"].volume = 0.95;
  }catch(e){ /* ignore */ }
}
function playSfx(name){
  try{
    const base = SFX[name];
    if(!base) return;
    // clone so rapid repeats can overlap
...
// ===== Sprite Animation System (Frame-by-frame PNG) =====
// Adora: IDLE @4fps (loop), Attack(light dagger) @12fps (once)
const SpriteDB = {
  adora: {
    idle:   { fps: 4,  loop: true,  frames: Array.from({length:7}, (_,i)=>`Sprites/adora/idle/adora_idle_${String(i).padStart(3,'0')}.png`) },
    attack: { fps: 12, loop: false, frames: Array.from({length:9}, (_,i)=>`Sprites/adora/attack/adora_attack_${String(i).padStart(3,'0')}.png`) },
    move:   { fps: 0,  loop: true,  frames: [ `Sprites/adora/move/Adora_Move.png` ] },
  }
};

// Preload sprite images and compute per-frame baseline offsets (to keep feet locked to tile).
// For each anim, we measure the transparent gap from the bottom to the lowest non-transparent pixel.
// We then align all frames to the smallest gap (the lowest feet) so feet never dip below that baseline.
const _spriteMeta = {}; // unitId -> animName -> {gaps[], minGap, pxScale}
function _computeBottomGap(img){
...
async function _preloadSpritePack(unitId){
  const pack = SpriteDB[unitId];
  if(!pack) return;
  if(_spriteMeta[unitId]) return;
  _spriteMeta[unitId] = {};
  for(const animName of Object.keys(pack)){
    const anim = pack[animName];
    const imgs = await Promise.all(anim.frames.map(_loadImage));
    const gaps = imgs.map(img=> img? _computeBottomGap(img) : 0);
    const minGap = Math.min(...gaps);
    const srcH = (imgs.find(Boolean)?.naturalHeight) || 256;
    const pxScale = 256 / srcH; // sprite element is 256px tall
    _spriteMeta[unitId][animName] = { gaps, minGap, pxScale };
  }
}


// ===== Camera helpers (zoom + focus)
let _camResetTimer = null;
function setCameraTransform({scale=1, tx=0, ty=0}={}){
  if(!battleStageEl) return;
  battleStageEl.style.setProperty('--cam-scale', String(scale));
  battleStageEl.style.setProperty('--cam-tx', `${tx}px`);
  battleStageEl.style.setProperty('--cam-ty', `${ty}px`);
}
function resetCamera(){
  if(_camResetTimer){ clearTimeout(_camResetTimer); _camResetTimer=null; }
  setCameraTransform({scale:1, tx:0, ty:0});
}
function focusCameraOnUnit(unit, {scale=1.35, holdMs=0}={}){
  const u = (typeof unit === 'string') ? units[unit] : unit;
  if(!u || !battleAreaEl) return;
  const p = getUnitCenterPoint(u);
  if(!p) return;
  const w = battleAreaEl.clientWidth || 1;
  const h = battleAreaEl.clientHeight || 1;
  // p is in battleArea local coords (see getCellCenter)
  // With transform: translate then scale => (p + t) * scale = center
  const tx = (w/(2*scale) - p.x);
  const ty = (h/(2*scale) - p.y);
  setCameraTransform({scale, tx, ty});
  if(holdMs>0){
    if(_camResetTimer) clearTimeout(_camResetTimer);
    _camResetTimer = setTimeout(()=>resetCamera(), holdMs);
  }
}

const _spriteState = {}; // unitId -> { anim, frame, acc }
function _getSpriteEl(unitId){
  const over = document.querySelector(`.units-layer .sprite[data-unit="${unitId}"]`);
  if(over) return over;
  return document.querySelector(`.unit[data-id="${unitId}"] .unit-sprite`);
}
function _ensureSpriteState(unitId){
  if(!_spriteState[unitId]) _spriteState[unitId] = { anim:'idle', frame:0, acc:0 };
  return _spriteState[unitId];
}
function _setSpriteFrame(unitId, animName, frameIndex){
  const pack = SpriteDB[unitId];
  if(!pack) return;
  const anim = pack[animName];
  if(!anim) return;
  const el = _getSpriteEl(unitId);
  if(!el) return;
  const safe = ((frameIndex%anim.frames.length)+anim.frames.length)%anim.frames.length;
  el.style.backgroundImage = `url("${anim.frames[safe]}")`;

  // Update per-frame foot lock offset (keeps feet from bobbing/dipping below the tile).
  const st = _ensureSpriteState(unitId);
  const meta = _spriteMeta?.[unitId]?.[animName];
  if(meta && meta.gaps && Number.isFinite(meta.minGap)){
    const gap = meta.gaps[safe] ?? meta.minGap;
    // gap difference in element pixels (sprite element is 256px tall)
    const px = (gap - meta.minGap) * (meta.pxScale || 1);
    st.footFixPx = px; // add to tile-bottom anchor when positioning
  } else {
    st.footFixPx = 0;
  }
}
function playUnitAnim(unitOrId, animName){
  const unitId = (typeof unitOrId === 'string') ? unitOrId : unitOrId?.id;
  if(!unitId || !SpriteDB[unitId]) return;
  const st = _ensureSpriteState(unitId);
  st.anim = animName;
  st.frame = 0;
  st.acc = 0;
  _setSpriteFrame(unitId, animName, 0);
}
function _tickSprites(dtMs){
  // Only animate units that have sprite packs (currently Adora)
  for(const unitId in SpriteDB){
    const u = units[unitId];
    if(!u || u.hp<=0) continue;

    const pack = SpriteDB[unitId];
    const st = _ensureSpriteState(unitId);
    const anim = pack[st.anim] || pack.idle;
    ...
    while(st.acc >= frameDur){
      ...
      if(st.frame >= anim.frames.length){
        if(anim.loop){
          ...
          // non-loop anim ends -> back to idle
          st.anim = 'idle';
          st.frame = 0;
        }
      }
      _setSpriteFrame(unitId, st.anim, st.frame);
    }
  }
}

let _spriteRafLast = null;
function startSpriteAnimator(){
  if(_spriteRafLast !== null) return; // already running
  _spriteRafLast = performance.now();
  const loop = (t)=>{
    const dt = Math.max(0, t - _spriteRafLast);
    _spriteRafLast = t;
    _tickSprites(dt);
...
function injectFXStyles(){
  ...
  .unit .unit-sprite { position:absolute; inset: 2px; background-repeat:no-repeat; background-position:center bottom; background-size: contain; pointer-events:none; }
  .unit[data-facing="left"] .unit-sprite { transform: scaleX(-1); }
  .unit[data-facing="right"] .unit-sprite { transform: scaleX(1); }
...
function ensureFxLayer(){
  if(!battleStageEl) return null;
  if(!fxLayer){
    fxLayer=document.createElement('div');
    fxLayer.className='fx-layer';
  }
  if(fxLayer.parentElement!==battleStageEl){
    battleStageEl.appendChild(fxLayer);
  }
  return fxLayer;
}
...
function showAttackFx({attacker=null, target=null, cell=null, point=null, trueDamage=false, heavy=false}={}){
  // Trigger attacker sprite animation (Adora light dagger swing)
  if(attacker && attacker.id === 'adora'){
    playUnitAnim(attacker,'attack');
    // Zoom camera onto Adora during the swing, then restore 0.5s after animation ends.
    const anim = SpriteDB?.adora?.attack;
    const animMs = anim ? (anim.frames.length / anim.fps) * 1000 : 0;
    focusCameraOnUnit(attacker,{scale:1.35, holdMs: Math.max(0, Math.round(animMs + 500))});
  }
  let anchor = null;
  if(target){
...
function showDeathFx(u){
  ...
  // Play global death SFX once per unit
  if(!u._deathSfxPlayed){
    playSfx('Death.mp3');
    u._deathSfxPlayed = true;
  }
...
function applyCameraTransform(){
  if(!battleStageEl) return;
  battleStageEl.style.setProperty('--cam-scale', cameraState.scale.toFixed(4));
  battleStageEl.style.setProperty('--cam-tx', `${cameraState.x.toFixed(2)}px`);
  battleStageEl.style.setProperty('--cam-ty', `${cameraState.y.toFixed(2)}px`);
}
...
function cameraShake(intensity='normal'){
  // IMPORTANT:
  // We must shake the SAME transformed container that holds BOTH the tile grid (.board)
  // and the overlay sprites (unitsLayer). If we shake only the grid, overlay sprites
  // (e.g., Adora) will appear to "not move with the map" during camera zoom/focus,
  // causing visible positional drift.
  if(!battleStageEl) return;
  const heavy = intensity==='heavy';
  const amp = heavy ? 5 : 2; // px
  const steps = heavy
    ? [[ amp, -amp],[-amp, amp],[ amp, amp],[-amp, -amp],[0,0]]
    : [[ amp, -amp],[-amp, amp],[ amp, amp],[-amp, -amp],[0,0]];

  // Cancel any existing shake by resetting vars first.
  battleStageEl.style.setProperty('--shake-x', '0px');
  battleStageEl.style.setProperty('--shake-y', '0px');

  let i = 0;
  const interval = heavy ? 55 : 40;
  const tick = ()=>{
    const s = steps[i] || [0,0];
    battleStageEl.style.setProperty('--shake-x', `${s[0]}px`);
    battleStageEl.style.setProperty('--shake-y', `${s[1]}px`);
    i++;
    if(i < steps.length) setTimeout(tick, interval);
    else {
      // Ensure clean reset
      battleStageEl.style.setProperty('--shake-x', '0px');
      battleStageEl.style.setProperty('--shake-y', '0px');
...
function adoraDagger(u,target){
  ...
  // Play Adora's attack animation + camera zoom during this skill
  if(u && u.id === 'adora'){
    playSfx('Slice 1.mp3');
    playUnitAnim(u,'attack');
    const anim = SpriteDB?.adora?.attack;
    const animMs = anim ? (anim.frames.length / anim.fps) * 1000 : 0;
    focusCameraOnUnit(u,{scale:1.35, holdMs: Math.max(0, Math.round(animMs + 500))});
  }
...
function darioClaw(u,target){
  ...
  // SFX: 50% Claw / 50% Claw 2
  playSfx(Math.random() < 0.5 ? 'Claw.mp3' : 'Claw 2.mp3');
...
async function darioPull(u, targetOrDesc){
  ...
  // SFX: Grabbing (trigger on hit)
  playSfx('Grabbing.mp3');
...
function karmaPunch(u,target){
  ...
  // SFX: Punch / Hard Punch / Critical Punch
  // Priority: 鸡血(critical) > 暴力瘾(连击加成) > normal
  if(u && u.status && u.status.jixueStacks>0){
    playSfx('Critical Punch.mp3');
  } else if((u.consecAttacks||0) >= 1){
    playSfx('Hard Punch.mp3');
  } else {
    playSfx('Punch.mp3');
  }
...
function buildSkillFactoriesForUnit(u){
  ...
  if(u.id==='adora'){
    ...
    F.push(
      ...
      { key:'枪击', prob:0.65, cond:()=>true, make:()=> skill('枪击',1,'gray','指定方向第一个单位 10HP+5SP',
        (uu,aimDir)=> aimDir? range_line(uu,aimDir) : (()=>{const a=[]; for(const d in DIRS) range_line(uu,d).forEach(x=>a.push(x)); return a;})(),
...
function buildGrid(){
  ...
  battleStageEl.style.setProperty('--cell', `${CELL_SIZE}px`);
...
function refreshLargeOverlays(){
  ...
  if(unitsLayerEl){
    // remove sprites for units that no longer exist or are dead
    unitsLayerEl.querySelectorAll('.sprite').forEach(sp=>{
      const id = sp.dataset.unit;
      if(!units[id] || units[id].hp<=0) sp.remove();
    });
  }
...
function placeUnits(){
  ...
  document.querySelectorAll('.cell').forEach(c=>c.classList.remove('occupied'));
  battleAreaEl.querySelectorAll('.largeOverlay').forEach(n=>n.remove());
  if(unitsLayerEl){
    // remove sprites for units that no longer exist or are dead
    unitsLayerEl.querySelectorAll('.sprite').forEach(sp=>{
  ...
  for(const id in units){
    ...
    div.dataset.id = id; // for sprite animation lookup
    div.dataset.facing = u.facing || 'right';

    // Scale up Adora sprite by +0.5x (1.5x total) while keeping feet anchored to the tile.
    // Adora scale (requested: shrink by half from 1.15 -> 0.575)
    if(id==='adora') div.style.setProperty('--unit-scale','0.5175');
    ...
    // create / position overlay sprite (visual can overflow tiles; clicks still on tile)
    if(SpriteDB[id] && unitsLayerEl){
      let sp = unitsLayerEl.querySelector(`.sprite[data-unit="${id}"]`);
      if(!sp){
        sp = document.createElement('div');
        sp.className = 'sprite';
        sp.dataset.unit = id;
        sp.dataset.facing = u.facing || 'right';
        unitsLayerEl.appendChild(sp);
      }
      // Scale: keep case-insensitive match for Adora
      const isAdora = String(id).toLowerCase() === 'adora';
      // User request: shrink further by 10% from prior half-size => 0.5175
      sp.style.setProperty('--unit-scale', (isAdora ? '0.5175' : (div.style.getPropertyValue('--unit-scale')||'1')));

      // Visual-only offset: Adora sprite down 10px (hitbox remains the tile)
      sp.style.setProperty('--unit-offset-y', isAdora ? '10px' : '0px');

      // --- Pixel-perfect anchoring to the actual cell DOM ---
      // Using constant math (CELL_SIZE/GRID_GAP/PADDING/BORDER) is fragile because CSS
      // borders, subpixel rounding, and browser layout can introduce 1px drift.
      // We instead anchor the overlay sprite to the real cell rect every time.
      const cellEl = getCellEl(u.r, u.c);
      if(cellEl){
        // IMPORTANT: Use offsetLeft/offsetTop instead of getBoundingClientRect.
        // getBoundingClientRect is affected by camera transforms (scale/translate), while
        // absolute positioning inside unitsLayer uses pre-transform layout coordinates.
        // Using offsets keeps the overlay sprite perfectly locked to the grid.
        const boardBorderX = battleAreaEl.clientLeft || 0; // border-left width
        const boardBorderY = battleAreaEl.clientTop  || 0; // border-top width
        const x = (battleAreaEl.offsetLeft + boardBorderX + cellEl.offsetLeft) + cellEl.offsetWidth/2;

        // Foot lock: per-frame baseline correction to keep feet stable and prevent dipping below tile bottom.
        const st = _spriteState[id];
        const footFix = (isAdora && st && st.footFixPx) ? st.footFixPx : 0;
        // Extra safety clamp upward so NO frame's feet ever go below the tile bottom.
        const BASE_FOOT_CLAMP_PX = isAdora ? 18 : 0;
        const y = (battleAreaEl.offsetTop + boardBorderY + cellEl.offsetTop + cellEl.offsetHeight) - BASE_FOOT_CLAMP_PX + footFix;

        // IMPORTANT: use subpixel values (no Math.round) so repeated moves don't accumulate drift.
        sp.style.left = `${x}px`;
        sp.style.top  = `${y}px`;
      }
      sp.dataset.facing = u.facing || 'right';
    ...
    // init sprite frame if available
    if(SpriteDB[id]){ _ensureSpriteState(id); _setSpriteFrame(id, _spriteState[id].anim, _spriteState[id].frame); }
...
function onCellClick(r,c){
  ...
  // Adora move pose during the 1s move transition
  if(String(sel.id).toLowerCase()==='adora'){ playUnitAnim('adora','move'); }

  sel.r=r; sel.c=c;
  registerUnitMove(sel);
  if(String(sel.id).toLowerCase()==='adora'){ setTimeout(()=>playUnitAnim('adora','idle'), 1000); }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  battleStageEl = document.getElementById('battleStage');
  unitsLayerEl = document.getElementById('unitsLayer');
  // Preload Adora sprites (baseline metadata)
  _preloadSpritePack('adora');
