...
function requestReturnToMenu(){
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'GW_DUO_BATTLE_FINISHED' }, '*');
      return;
    }
  } catch (e) {
    // ignore
...
window.addEventListener('message', (event) => {
  ...
  if (!data || data.type !== 'GW_DUO_SELECTED_SKILLS') return;
  if (data.selections && typeof data.selections === 'object') {
    __gwInjectedSkillSelection = { mode: 'duo', data: data.selections };
  }
});
...
function getSelectedSkillKeysForUnit(u) {
  ...
  if (selectedSkills.mode === 'duo') {
    ...
    charSelection = (selectedSkills && selectedSkills.data && selectedSkills.data[playerKey] ? selectedSkills.data[playerKey][baseId] : undefined);
  } else {
    charSelection = (selectedSkills && selectedSkills.data ? selectedSkills.data[baseId] : undefined);
  }
