// 2D 回合制 RPG Demo - 西斯特 Boss 战
// 变更摘要：
// - 新增西斯特单体 Boss 战：29x20 画布场地，按玩家手绘地图布置。
// - 文档机制第一批实现：七种特殊颜色状态、颜料/颜料印记、色系混合、灵感、完成的作品。
// - 二阶段“意识到了”：HP≤200 后切换技能池，负SP、90%SP防御、颜色转黑、发布机制。
// - 保留原战斗框架与AI寻路，新增西斯特专属技能池与主菜单入口。
...
const SISTER_PHASE_HP = 200;
const SISTER_SP_CRASH_FLOOR = -45;
const SISTER_SP_DEFENSE_THRESHOLD = -35;
...
let sisterBGMPhase1 = null;
let sisterBGMPhase2 = null;
let sisterCurrentBGM = null;
let sisterCurrentBGMPhase = 0;
let sisterBGMUnlocked = false;
let sisterPendingBGMPhase = 0;
let sisterMusicUnlockTip = null;
let startBattleOverlay = null;
...
function addCoverRectRC(r1,c1,r2,c2){
...
function addCoverOutlineRectRC(r1,c1,r2,c2){
  const rmin = Math.min(r1,r2), rmax = Math.max(r1,r2);
  const cmin = Math.min(c1,c2), cmax = Math.max(c1,c2);
  for(let r=rmin; r<=rmax; r++){
    for(let c=cmin; c<=cmax; c++){
      const isEdge = (r===rmin || r===rmax || c===cmin || c===cmax);
      if(isEdge && r>=1 && r<=ROWS && c>=1 && c<=COLS && !isVoidCell(r,c)){
        coverCells.add(`${r},${c}`);
      }
    }
  }
}
// 西斯特手绘地图：坐标按图片上方/左侧标注，左上角为(1,1)，横向1～29，纵向1～20。
// 图里的六个矩形是实心掩体/画框区域，不能进入，不能中间留洞。
// 左侧三块掩体
addCoverRectRC(4,5,6,10);
addCoverRectRC(9,5,11,10);
addCoverRectRC(14,5,16,10);
// 右侧三块掩体
addCoverRectRC(4,20,6,25);
addCoverRectRC(9,20,11,25);
addCoverRectRC(14,20,16,25);
...
const units = {};
// 玩家（西斯特战：35级三人组）
units['adora'] = createUnit('adora','Adora','player',35, 19, 15, 100,100, 0.5,0, ['backstab','calmAnalysis','proximityHeal','fearBuff']);
...
units['sister'] = createUnit('sister','西斯特','enemy',45, 2, 15, 550, 125, 0, 0, [
  ...
  size:1,
  stunThreshold:4,
  spFloor:SISTER_SP_CRASH_FLOOR,
  disableSpCrash:true,
  initialSp:125,
  pullImmune:true,
});
// initSisterState 会在西斯特颜色常量定义后执行，避免 const 初始化前访问。
...
function injectFXStyles(){
  ...
  .cell.highlight-publish-zone { background: rgba(91,22,126,0.30) !important; box-shadow: inset 0 0 14px rgba(185,92,255,0.42); }
  .cell.highlight-publish-left { background: rgba(211,82,255,0.40) !important; box-shadow: inset 0 0 18px rgba(255,196,255,0.50); }
  .cell.highlight-publish-right { background: rgba(116,88,255,0.40) !important; box-shadow: inset 0 0 18px rgba(197,191,255,0.54); }
  .cell.highlight-publish-center { background: rgba(110,36,145,0.45) !important; box-shadow: inset 0 0 18px rgba(209,145,255,0.55); }
  .cell.highlight-publish-slam { background: rgba(255,255,255,0.18) !important; box-shadow: inset 0 0 22px rgba(209,145,255,0.75), 0 0 10px rgba(209,145,255,0.30); }
  .unit-travel-hidden { opacity: 0.18 !important; filter: blur(1px) saturate(0.7); }
  .unit-dash-ghost { position:absolute; z-index:1450; pointer-events:none; transform:translate(-50%,-50%); transition: transform var(--travel-ms, 520ms) cubic-bezier(.18,.88,.24,1.02), opacity 180ms ease; filter: drop-shadow(0 0 18px rgba(183,104,255,0.65)); }
  .unit-dash-ghost.player { background: rgba(45,105,255,0.95); }
  .unit-dash-ghost.enemy { background: rgba(255,80,110,0.95); }
  .unit-dash-ghost.teleport { animation: ghost-blink 190ms ease-in-out infinite alternate; }
  .unit-dash-ghost.sister-cine { z-index:1750; filter: drop-shadow(0 0 22px rgba(220,150,255,0.95)) drop-shadow(0 0 44px rgba(93,29,255,0.55)); }
  .sister-slash-lane { position:absolute; z-index:1660; pointer-events:none; border-radius:999px; height:10px; min-width:46px;
    background:linear-gradient(90deg, rgba(255,255,255,0), rgba(252,228,255,.96), rgba(151,54,255,.94), rgba(255,255,255,0));
    box-shadow:0 0 16px rgba(210,124,255,.92), 0 0 38px rgba(110,42,255,.58); transform-origin:50% 50%; opacity:0; animation:sister-lane-cut 420ms cubic-bezier(.08,.72,.16,1) forwards; }
  .sister-point-cut { position:absolute; z-index:1665; pointer-events:none; width:72px; height:72px; transform:translate(-50%,-50%) rotate(var(--cut-angle, -24deg)); opacity:0;
    background:linear-gradient(90deg, rgba(255,255,255,0), rgba(255,241,255,.95), rgba(148,62,255,.92), rgba(255,255,255,0));
    clip-path:polygon(0 45%, 82% 37%, 100% 50%, 82% 63%, 0 55%); filter:drop-shadow(0 0 16px rgba(193,91,255,.9)); animation:sister-point-cut 380ms ease-out forwards; }
  @keyframes sister-lane-cut { 0%{ opacity:0; transform:translate(-50%,-50%) rotate(var(--lane-angle,0deg)) scaleX(.18); } 20%{ opacity:1; } 68%{ opacity:1; transform:translate(-50%,-50%) rotate(var(--lane-angle,0deg)) scaleX(1.05); } 100%{ opacity:0; transform:translate(-50%,-50%) rotate(var(--lane-angle,0deg)) scaleX(.52); } }
  @keyframes sister-point-cut { 0%{ opacity:0; transform:translate(-50%,-50%) rotate(var(--cut-angle, -24deg)) scale(.45); } 28%{ opacity:1; } 100%{ opacity:0; transform:translate(-50%,-50%) rotate(var(--cut-angle, -24deg)) scale(1.12); } }
  .sister-signature-seal { position:absolute; z-index:1680; pointer-events:none; width:96px; height:96px; left:0; top:0; transform:translate(-50%,-50%) scale(.72) rotate(-12deg); opacity:0;
    border:2px solid rgba(24,0,33,.82); border-radius:16px; box-shadow:inset 0 0 18px rgba(208,118,255,.52), 0 0 26px rgba(171,64,255,.68);
    background:radial-gradient(circle, rgba(236,202,255,.20), rgba(55,0,75,.38) 46%, rgba(0,0,0,0) 72%); animation:sister-signature-seal 700ms ease-out forwards; }
  .sister-signature-seal::before { content:'签'; position:absolute; inset:0; display:flex; align-items:center; justify-content:center; color:#f8eaff; font-size:42px; font-weight:1000; text-shadow:0 0 10px #000, 0 0 18px rgba(219,146,255,.95); }
  .sister-signature-bloom { position:absolute; z-index:1675; pointer-events:none; width:12px; height:12px; left:0; top:0; border-radius:50%; opacity:0; background:var(--sig-color,#fff); box-shadow:0 0 14px var(--sig-color,#fff); animation:sister-signature-bloom 620ms ease-out forwards; }
  @keyframes sister-signature-seal { 0%{opacity:0; transform:translate(-50%,-50%) scale(.42) rotate(-20deg);} 28%{opacity:1; transform:translate(-50%,-50%) scale(1.06) rotate(-7deg);} 100%{opacity:0; transform:translate(-50%,-50%) scale(1.18) rotate(4deg);} }
  @keyframes sister-signature-bloom { 0%{opacity:0; transform:translate(-50%,-50%) scale(.35);} 24%{opacity:1;} 100%{opacity:0; transform:translate(calc(-50% + var(--dx,0px)), calc(-50% + var(--dy,0px))) scale(1.15);} }
  @keyframes ghost-blink { from{ opacity:0.58; } to{ opacity:1; } }
  .cell.highlight-path { background: rgba(137, 86, 255, 0.20) !important; box-shadow: inset 0 0 12px rgba(177,115,255,0.36); }
  .sister-cinematic-caption { position:absolute; left:50%; top:12%; transform:translate(-50%,-50%); z-index:1400; pointer-events:none;
    padding:10px 18px; border:1px solid rgba(214,164,255,0.72); border-radius:14px; color:#f8eaff; font-weight:900; letter-spacing:3px;
    background:linear-gradient(90deg, rgba(21,0,35,0.08), rgba(76,18,111,0.86), rgba(21,0,35,0.08));
    text-shadow:0 0 14px rgba(198,96,255,0.95), 0 1px 0 #000; animation:sister-caption-fade 980ms ease-out forwards; }
  .sister-screen-flash { position:absolute; inset:0; z-index:1350; pointer-events:none; mix-blend-mode:screen;
    background:radial-gradient(circle, rgba(236,198,255,0.42) 0%, rgba(133,41,255,0.20) 45%, rgba(0,0,0,0) 78%); animation:sister-screen-flash 560ms ease-out forwards; }
  @keyframes sister-caption-fade { 0%{ opacity:0; transform:translate(-50%,-58%) scale(0.94); } 20%{ opacity:1; transform:translate(-50%,-50%) scale(1.03); } 100%{ opacity:0; transform:translate(-50%,-42%) scale(1.08); } }
  @keyframes sister-screen-flash { 0%{ opacity:0; } 22%{ opacity:1; } 100%{ opacity:0; } }

  .sister-publish-sweep { position:absolute; z-index:1650; pointer-events:none; border:1px solid rgba(233,190,255,0.26); border-radius:10px; overflow:visible; mix-blend-mode:screen; }
  .sister-publish-sweep .blade { position:absolute; left:50%; top:50%; width:var(--blade-len, 260px); height:8px; border-radius:999px;
    background:linear-gradient(90deg, rgba(255,255,255,0), rgba(246,210,255,0.92), rgba(166,61,255,0.96), rgba(255,255,255,0));
    box-shadow:0 0 18px rgba(213,123,255,0.90), 0 0 42px rgba(117,38,255,0.70); transform-origin:50% 50%;
    animation:sister-blade-sweep 520ms cubic-bezier(.1,.7,.2,1) forwards; }
  .sister-publish-sweep.reverse .blade { animation-name:sister-blade-sweep-rev; }
  .sister-publish-sweep .cutline { position:absolute; inset:8%; border-radius:10px; border:1px solid rgba(255,225,255,0.36); opacity:0;
    box-shadow:inset 0 0 28px rgba(193,91,255,0.30); animation:sister-cutline-pop 520ms ease-out forwards; }
  .sister-publish-ghost { position:absolute; z-index:1700; pointer-events:none; width:42px; height:42px; border-radius:10px;
    background:linear-gradient(135deg, rgba(255,88,120,0.95), rgba(91,28,159,0.98)); color:#fff; font-size:11px; font-weight:900;
    display:flex; align-items:center; justify-content:center; border:1px solid rgba(255,214,255,0.78);
    text-shadow:0 1px 4px #000; box-shadow:0 0 20px rgba(193,91,255,0.80), 0 0 52px rgba(84,23,255,0.50);
    transform:translate(-50%,-50%) scale(0.85); opacity:0; transition:transform 360ms cubic-bezier(.1,.85,.18,1.05), opacity 100ms ease; }
  .sister-publish-blink { position:absolute; z-index:1500; pointer-events:none; width:52px; height:52px; border-radius:50%; transform:translate(-50%,-50%) scale(0.35); opacity:0;
    background:radial-gradient(circle, rgba(238,203,255,0.92), rgba(155,69,255,0.42) 42%, rgba(0,0,0,0) 72%); mix-blend-mode:screen;
    animation:sister-publish-blink 260ms ease-out forwards; }
  .sister-area-smash { position:absolute; z-index:1640; pointer-events:none; border-radius:12px; mix-blend-mode:screen;
    background:radial-gradient(circle, rgba(255,255,255,0.24), rgba(174,74,255,0.34) 42%, rgba(0,0,0,0) 78%);
    box-shadow:inset 0 0 42px rgba(231,192,255,0.55), 0 0 38px rgba(161,65,255,0.45);
    transform:scale(1.25); opacity:0; animation:sister-area-smash 520ms ease-out forwards; }
  @keyframes sister-blade-sweep { 0%{ opacity:0; transform:translate(-96%,-50%) rotate(var(--blade-angle,0deg)) scaleX(0.25); } 20%{ opacity:1; } 70%{ opacity:1; transform:translate(-50%,-50%) rotate(var(--blade-angle,0deg)) scaleX(1.05); } 100%{ opacity:0; transform:translate(-4%,-50%) rotate(var(--blade-angle,0deg)) scaleX(0.45); } }
  @keyframes sister-blade-sweep-rev { 0%{ opacity:0; transform:translate(-4%,-50%) rotate(var(--blade-angle,0deg)) scaleX(0.25); } 20%{ opacity:1; } 70%{ opacity:1; transform:translate(-50%,-50%) rotate(var(--blade-angle,0deg)) scaleX(1.05); } 100%{ opacity:0; transform:translate(-96%,-50%) rotate(var(--blade-angle,0deg)) scaleX(0.45); } }
  @keyframes sister-cutline-pop { 0%{ opacity:0; transform:scale(0.88); } 35%{ opacity:0.9; } 100%{ opacity:0; transform:scale(1.10); } }
  @keyframes sister-publish-blink { 0%{ opacity:0; transform:translate(-50%,-50%) scale(0.22); } 30%{ opacity:1; } 100%{ opacity:0; transform:translate(-50%,-50%) scale(1.25); } }
  @keyframes sister-area-smash { 0%{ opacity:0; transform:scale(1.35); filter:blur(2px); } 35%{ opacity:1; transform:scale(0.98); filter:blur(0); } 100%{ opacity:0; transform:scale(1.10); } }
...
function getCellCenter(r,c){
  ...
  // IMPORTANT: FX are appended inside #battleArea, which is the same element
  // being camera-transformed.  Using getBoundingClientRect() here returns
  // already-scaled / already-translated screen coordinates, so after the
  // camera moves or zooms, slash FX and dash ghosts drift away from the grid.
  // Use local board coordinates instead, so the camera transform moves the
  // board and FX together.
  return {
    x: cell.offsetLeft + cell.offsetWidth / 2,
    y: cell.offsetTop + cell.offsetHeight / 2,
  };
}
function getCellScreenCenter(r,c){
  const cell = getCellEl(r,c);
  if(!cell) return {x:0,y:0};
...
const SKILL_FX_CONFIG = {
  ...
  'sister:随性作画':       {type:'slash', primary:'#ff7ad9', secondary:'rgba(190,95,255,0.70)', spark:'#ffe5ff', slashes:3, attack:{type:'swing', swings:2, spread:18, delayStep:34, variant:'wide'}},
  'sister:剧本创作':       {type:'cascade', primary:'#d8a1ff', secondary:'#8057ff', droplets:8},
  'sister:签名':           {type:'rune', primary:'#222222', secondary:'#b06cff', glyph:'签'},
  'sister:发泄性作画':     {type:'claw', primary:'#2b0f45', secondary:'#8b46ff', scratches:5, spacing:17, shards:4, variant:'sister'},
  'sister:发布':           {type:'burst', primary:'#0a0015', secondary:'#b469ff', glyph:'■', duration:900, attack:{type:'swing', swings:4, spread:32, delayStep:42, variant:'wide', faceTarget:false}},
  'sister:发布·斩':        {type:'slash', primary:'#16001f', secondary:'rgba(184,79,255,0.86)', spark:'#f0d7ff', slashes:4, attack:{type:'swing', swings:4, spread:36, delayStep:34, variant:'wide', faceTarget:false}},
  'sister:发布·坠':        {type:'impact', primary:'#1a0526', secondary:'#cf93ff'},
};
...
function refreshSpCrashVulnerability(u){
  ...
  const canClearSisterCrash = u.id === 'sister' && stunnedStacks <= 0;
  const canClearNormalCrash = u.id !== 'sister' && stunnedStacks <= 0 && u.sp > 0;
  if(u._spCrashVuln && (canClearSisterCrash || canClearNormalCrash)){
...
function sisterAddTemporaryDamageBoost(u, amount=1, reason=''){
  if(!u || !u.status) return 0;
  const add = Math.max(0, Math.floor(Number(amount) || 0));
  if(add <= 0) return u.status.sisterDamageBoost || 0;
  const next = (u.status.sisterDamageBoost || 0) + add;
  updateStatusStacks(u, 'sisterDamageBoost', next, {label:'增伤', type:'buff'});
  if(reason) appendLog(`${u.name} ${reason}：增伤 +${add}（当前 ${next} 层，下次造成HP伤害后消耗）`);
  return next;
}
function sisterAddPermanentDamageBoost(u, amount=1, reason=''){
  // 兼容旧调用名：西斯特的“增伤”不是永久Buff，会在下一次造成HP伤害时消耗。
  return sisterAddTemporaryDamageBoost(u, amount, reason);
}
function sisterConsumeDamageBoostForAttack(source, opts={}){
  if(!source || !source.status) return 0;
  const boost = Math.max(0, Math.floor(Number(source.status.sisterDamageBoost)||0));
  if(boost <= 0) return 0;
  const token = opts && opts.sisterBoostToken;
  if(token){
    if(!source._sisterBoostTokenCache) source._sisterBoostTokenCache = {};
    if(source._sisterBoostTokenCache[token] !== undefined){
      return source._sisterBoostTokenCache[token];
    }
    source._sisterBoostTokenCache[token] = boost;
  }
  updateStatusStacks(source, 'sisterDamageBoost', 0, {label:'增伤', type:'buff'});
  appendLog(`${source.name} 的增伤触发：本段HP伤害 +${boost*10}%，增伤已消耗`);
  return boost;
}
...
function normalizeBleedState(u){
  ...
  // 新流血体系：层数与强度分开记录。
  // 普通“造成流血”只保证两者至少为1；只有明确写“+层数/+强度”才增加对应数值。
  if(layers <= 0 && strength <= 0){
    u.status.bleed = 0;
  ...
  u.status.bleed = layers;
  u.status.bleedStrength = strength;
  // 仅作兼容显示/旧代码读取；伤害计算不再把这里当总强度相加。
...
function setBleedStacksDirect(u, layers){
  ...
  u.status.bleedStrength = strength;
  u.status.bleedStrengthQueue = Array.from({length: nextLayers}, ()=>strength);
}
function ensureBleed(u){
  if(!u || !u.status) return 0;
  let layers = Math.max(0, Math.floor(Number(u.status.bleed)||0));
  let strength = Math.max(0, Math.floor(Number(u.status.bleedStrength)||0));
  if(layers <= 0) layers = 1;
...
function tickBleedDamage(u){
  if(!u || !u.status || !(u.status.bleed>0)) return;
  normalizeBleedState(u);
  const bleedLayersBefore = Math.max(0, Math.floor(Number(u.status.bleed)||0));
  const bleedStrengthBefore = Math.max(0, Math.floor(Number(u.status.bleedStrength)||0));
  const bleedDmg = Math.max(0, bleedLayersBefore * bleedStrengthBefore * 2);
  if(bleedDmg > 0){
    damageUnit(u.id, bleedDmg, 0, `${u.name} 因流血受损（${bleedLayersBefore}层×强度${bleedStrengthBefore}×2HP）`, null, {trueDamage:true});
  }
  const bleedLayersAfter = Math.max(0, bleedLayersBefore - 1);
  if(bleedLayersAfter <= 0){
    u.status.bleed = 0;
    u.status.bleedStrength = 0;
    u.status.bleedStrengthQueue = [];
    updateStatusStacks(u, 'bleed', 0, {label:'流血', type:'debuff'});
  }else{
    u.status.bleedStrength = bleedStrengthBefore;
    u.status.bleedStrengthQueue = Array.from({length: bleedLayersAfter}, ()=>bleedStrengthBefore);
...
// —— 西斯特：颜色状态 / 颜料 / 二阶段 ——
const SISTER_COLOR_KEYS = ['red','blue','yellow','orange','green','purple','black'];
const SISTER_COLOR_LABELS = { red:'红色', blue:'蓝色', yellow:'黄色', orange:'橙色', green:'绿色', purple:'紫色', black:'黑色' };
const SISTER_PRIMARY_COLORS = ['red','blue','yellow'];
...
function initSisterState(u){
  ...
  u.status.paint = u.status.paint || 0;
  u.status.inspiration = u.status.inspiration || 0;
  u.status.completedWorks = u.status.completedWorks || 0;
  u.status.sisterDamageBoost = u.status.sisterDamageBoost || 0;
  u._sisterAwakened = false;
  u._sisterSkillHitsTaken = 0;
  u._sisterHitCounter = 0;
  u._sisterSignedTargets = u._sisterSignedTargets || {};
  u._sisterCumulativeBlack = 0;
}
initSisterState(units['sister']);

function ensureSisterColors(u){
  if(!u) return null;
  if(!u.sisterColors) u.sisterColors = {};
  for(const key of SISTER_COLOR_KEYS){
    if(!u.sisterColors[key]) u.sisterColors[key] = {layers:0, strength:0};
    u.sisterColors[key].layers = Math.max(0, Math.floor(Number(u.sisterColors[key].layers)||0));
    u.sisterColors[key].strength = Math.max(0, Math.floor(Number(u.sisterColors[key].strength)||0));
  }
  if(!u._sisterMixMemory) u._sisterMixMemory = {purple:0,orange:0,green:0,black:0};
  return u.sisterColors;
}
...
function sisterHasAnyColor(u){ return sisterTotalColorLayers(u,true) > 0; }
function sisterAddPaint(u, layers=1, reason='颜料'){
  if(!u || !u.status) return 0;
  const add = Math.max(0, Math.floor(Number(layers)||0));
  if(add<=0) return u.status.paint || 0;
  u.status.paint = Math.max(0, (u.status.paint||0) + add);
  showStatusFloat(u,'颜料',{type:'buff',delta:add,offsetY:-96});
  if(reason) appendLog(`${u.name} 获得 ${add} 层颜料（${reason}，当前 ${u.status.paint}）`);
  return u.status.paint;
}
function sisterSpendPaint(u, layers=1){
  if(!u || !u.status) return false;
  const need = Math.max(1, Math.floor(Number(layers)||1));
  if((u.status.paint||0) < need){ appendLog(`${u.name} 颜料不足，无法使用此技能`); return false; }
  u.status.paint = Math.max(0, (u.status.paint||0) - need);
  showStatusFloat(u,'颜料',{type:'debuff',delta:-need,offsetY:-96});
  return true;
}
...
function sisterConsumeColor(u, key, layers=1){
  ...
  const avg = colors[key].layers > 0 ? Math.max(1, Math.round(colors[key].strength / colors[key].layers)) : 1;
  colors[key].layers = Math.max(0, colors[key].layers - count);
  colors[key].strength = Math.max(0, colors[key].strength - avg * count);
  showStatusFloat(u,SISTER_COLOR_LABELS[key],{type:'debuff',delta:-count,offsetY:-104});
  sisterApplyColorMixing(u); // 颜色被消耗后，混色记忆也要跟着重新校准
...
function sisterApplyColorMixing(u){
  ...
  try{
    const colors=ensureSisterColors(u);
    // 混色不是“历史最高值永久锁死”。
    // 如果原色/混合色被消耗，记忆也要降回当前可混合量；之后重新叠回颜色时才能再次混色。
    const addIfNew = (out, amount, strength=1)=>{
      amount = Math.max(0, Math.floor(Number(amount)||0));
      const prev = u._sisterMixMemory[out] || 0;
      if(amount < prev){
        u._sisterMixMemory[out] = amount;
        return;
      }
      if(amount > prev){
        const delta = amount - prev;
        u._sisterMixMemory[out] = amount;
        sisterAddColor(u, out, delta, strength, '色系混合');
      }
    };
    addIfNew('purple', Math.min(colors.red.layers, colors.blue.layers), 1);
    addIfNew('orange', Math.min(colors.red.layers, colors.yellow.layers), 1);
    addIfNew('green',  Math.min(colors.yellow.layers, colors.blue.layers), 1);
    addIfNew('black',  Math.min(colors.purple.layers, colors.orange.layers, colors.green.layers), 1);
  }finally{ u._sisterMixing = false; }
}
function sisterEligibleForSignature(target){
  if(!target || target.hp<=0) return false;
  const colors=ensureSisterColors(target);
  return ['red','blue','yellow','orange','green','purple'].every(k=>colors[k].layers>0) && !(units['sister'] && units['sister']._sisterSignedTargets && units['sister']._sisterSignedTargets[target.id]);
}
function sisterSignatureEligibleTargets(){ return Object.values(units).filter(u=>u && u.side==='player' && sisterEligibleForSignature(u)); }
function sisterRandomColor(){ return SISTER_COLOR_KEYS[Math.floor(Math.random()*SISTER_COLOR_KEYS.length)]; }
function sisterRandomPrimaryColor(){ return SISTER_PRIMARY_COLORS[Math.floor(Math.random()*SISTER_PRIMARY_COLORS.length)]; }
function sisterRandomOpenCellAround(unit, radius=2){
  if(!unit) return null;
  const cells=[];
  for(let r=Math.max(1, unit.r-radius); r<=Math.min(ROWS, unit.r+radius); r++){
    for(let c=Math.max(1, unit.c-radius); c<=Math.min(COLS, unit.c+radius); c++){
      if(!clampCell(r,c)) continue;
...
function sisterRandomEmptyCell(){
  const cells=[];
  for(let r=1;r<=ROWS;r++) for(let c=1;c<=COLS;c++) if(clampCell(r,c) && !getUnitAt(r,c)) cells.push({r,c});
  return cells.length ? cells[Math.floor(Math.random()*cells.length)] : null;
}
function sisterTeleportUnit(u, dest, label='传送'){
  if(!u || !dest) return false;
  const old = {r:u.r,c:u.c};
  showTrailWithDuration(old.r, old.c, dest.r, dest.c, 500, {thickness:8, color:'rgba(180,80,255,0.85)'});
  u.r = dest.r; u.c = dest.c; pulseCell(u.r,u.c); cameraFocusOnCell(u.r,u.c,{scale:1.15,hold:260});
  appendLog(`${u.name} ${label}：(${old.r},${old.c}) → (${u.r},${u.c})`);
  return true;
}

async function animateUnitTravel(u, dest, label='移动', opts={}){
  if(!u || !dest || !clampCell(dest.r, dest.c)) return false;
  const old = {r:u.r, c:u.c};
  if(old.r === dest.r && old.c === dest.c) return true;
  const distance = Math.max(Math.abs(dest.r-old.r), Math.abs(dest.c-old.c));
  const isLongBlink = !!opts.teleport || distance > 4 || opts.noLongTrail;
  const duration = opts.duration || (isLongBlink ? 240 : 320);
  const color = opts.color || (u.side === 'enemy' ? 'rgba(180,80,255,0.88)' : 'rgba(88,153,255,0.88)');
  const from = getUnitCenterPoint(u) || getCellCenter(old.r, old.c);
  const to = getCellCenter(dest.r, dest.c);
  const layer = ensureFxLayer();
  const realEl = document.querySelector(`.unit[data-id="${u.id}"]`) || document.querySelector(`.largeOverlay[data-id="${u.id}"]`);

  // 长距离/瞬移类动作不再画很长的行动轨迹：只做起点消失 + 终点出现，避免尴尬拖线与卡顿。
  if(isLongBlink){
    cameraFocusOnCell(old.r, old.c, {scale:opts.startScale || 1.10, hold:90});
    if(layer){
      const a=document.createElement('div'); a.className='sister-publish-blink'; a.style.left=`${from.x}px`; a.style.top=`${from.y}px`; layer.appendChild(a); setTimeout(()=>a.remove(),340);
    }
    if(realEl) realEl.classList.add('unit-travel-hidden');
    await sleep(Math.min(120, duration));
    u.r = dest.r; u.c = dest.c;
    renderAll();
    const p2=getCellCenter(dest.r,dest.c);
    if(layer){
      const b=document.createElement('div'); b.className='sister-publish-blink'; b.style.left=`${p2.x}px`; b.style.top=`${p2.y}px`; layer.appendChild(b); setTimeout(()=>b.remove(),340);
    }
    cameraFocusOnCell(dest.r, dest.c, {scale:opts.endScale || 1.16, hold:140});
    if(opts.shake) cameraShake(opts.shake === 'heavy' ? 'heavy' : 'normal');
    pulseCell(u.r,u.c);
    await sleep(Math.max(90, duration-100));
    const newEl = document.querySelector(`.unit[data-id="${u.id}"]`) || document.querySelector(`.largeOverlay[data-id="${u.id}"]`);
    if(newEl) newEl.classList.remove('unit-travel-hidden');
    appendLog(`${u.name} ${label}：(${old.r},${old.c}) → (${u.r},${u.c})`);
    await sleep(opts.after || 60);
    return true;
  }

  const bounds = getUnitBounds(u);
  const pathCells = approximatePathCells(old, dest).slice(0, 5);
  if(pathCells.length && opts.path !== false) addTempClassToCells(pathCells, 'highlight-path', duration + 100);
  if(opts.trail !== false) showTrailWithDuration(old.r, old.c, dest.r, dest.c, Math.max(duration, 260), {thickness:opts.thickness || 5, color});
  cameraFocusOnCell(dest.r, dest.c, {scale:opts.endScale || 1.14, hold:duration});
  let ghost = null;
  if(layer){
    ghost = document.createElement('div');
    ghost.className = `unit unit-dash-ghost ${u.side==='player'?'player':'enemy'}`;
    ghost.dataset.facing = u.facing || 'right';
    ghost.innerHTML = realEl ? realEl.innerHTML : `<div>${u.name}</div>`;
    ghost.style.left = `${from.x}px`;
    ghost.style.top = `${from.y}px`;
    ghost.style.right = 'auto';
    ghost.style.bottom = 'auto';
    ghost.style.width = `${bounds ? bounds.width : 36}px`;
    ghost.style.height = `${bounds ? bounds.height : 36}px`;
    ghost.style.setProperty('--travel-ms', `${duration}ms`);
  ...
  if(ghost){
    ...
    ghost.style.transform = `translate(-50%,-50%) translate(${dx}px, ${dy}px) scale(${opts.scale || 1.03})`;
  }
  if(opts.shake) cameraShake(opts.shake === 'heavy' ? 'heavy' : 'normal');
  await sleep(duration);
  if(realEl) realEl.classList.remove('unit-travel-hidden');
...
function approximatePathCells(from, to){
  const cells=[];
  if(!from || !to) return cells;
  let r=from.r, c=from.c;
  const dr = Math.sign(to.r - r);
  const dc = Math.sign(to.c - c);
  while(c !== to.c){ c += dc; if(clampCell(r,c)) cells.push({r,c}); }
  while(r !== to.r){ r += dr; if(clampCell(r,c)) cells.push({r,c}); }
  return cells;
}
async function sisterTeleportUnitAnimated(u, dest, label='传送', opts={}){
  return animateUnitTravel(u, dest, label, Object.assign({duration:opts.duration || 260, color:'rgba(180,80,255,0.92)', teleport:true, thickness:6, startScale:1.10, endScale:1.16, noLongTrail:true}, opts));
}
function sisterSpawnDashGhost(u, fromCell, toCell, opts={}){
  const layer=ensureFxLayer(); if(!layer || !u || !fromCell || !toCell) return null;
  const from=getCellCenter(fromCell.r, fromCell.c);
  const to=getCellCenter(toCell.r, toCell.c);
  const realEl=document.querySelector(`.unit[data-id="${u.id}"]`) || document.querySelector(`.largeOverlay[data-id="${u.id}"]`);
  const bounds=getUnitBounds(u);
  const duration = opts.duration || 320;
  const ghost=document.createElement('div');
  ghost.className=`unit unit-dash-ghost sister-cine ${u.side==='player'?'player':'enemy'}`;
  ghost.dataset.facing = opts.facing || u.facing || 'right';
  ghost.innerHTML = realEl ? realEl.innerHTML : `<div>${u.name}</div>`;
  ghost.style.left=`${from.x}px`; ghost.style.top=`${from.y}px`;
  ghost.style.right='auto'; ghost.style.bottom='auto';
  ghost.style.width=`${bounds ? bounds.width : 38}px`;
  ghost.style.height=`${bounds ? bounds.height : 38}px`;
  ghost.style.setProperty('--travel-ms', `${duration}ms`);
  layer.appendChild(ghost);
  if(opts.hideReal && realEl) realEl.classList.add('unit-travel-hidden');
  requestAnimationFrame(()=>{
    ghost.style.opacity='1';
    ghost.style.transform=`translate(-50%,-50%) translate(${to.x-from.x}px, ${to.y-from.y}px) scale(${opts.scale || 1.07})`;
  });
  setTimeout(()=>{
    ghost.style.opacity='0';
    if(opts.hideReal && realEl) realEl.classList.remove('unit-travel-hidden');
    setTimeout(()=>ghost.remove(),180);
  }, duration + (opts.linger || 140));
  return ghost;
}
function sisterSlashLaneFx(cells, dir, opts={}){
  const layer=ensureFxLayer(); const bounds=sisterCellsBounds(cells); if(!layer || !bounds) return null;
  const el=document.createElement('div');
  el.className='sister-slash-lane';
  const len=Math.max(46, Math.hypot(bounds.width, bounds.height));
  el.style.left=`${bounds.centerX}px`; el.style.top=`${bounds.centerY}px`;
  el.style.width=`${len * (opts.lenMul || 1.18)}px`;
  const angle = typeof opts.angle === 'number' ? opts.angle : sisterLateralAngleForDir(dir||'up');
  el.style.setProperty('--lane-angle', `${angle}deg`);
  layer.appendChild(el); setTimeout(()=>el.remove(),520); return el;
}
function sisterPointSlashFx(cell, dir, opts={}){
  const layer=ensureFxLayer(); if(!layer || !cell) return null;
  const p=getCellCenter(cell.r, cell.c);
  const el=document.createElement('div'); el.className='sister-point-cut';
  el.style.left=`${p.x}px`; el.style.top=`${p.y}px`;
  const angleMap={right:0,left:180,up:-90,down:90};
  const base=angleMap[dir||'right'] ?? 0;
  el.style.setProperty('--cut-angle', `${base + (opts.flip ? 20 : -20)}deg`);
  layer.appendChild(el); setTimeout(()=>el.remove(),460); return el;
}
function sisterSignatureBloomFx(target){
  const layer=ensureFxLayer(); if(!layer || !target) return;
  const p=getCellCenter(target.r,target.c);
  const colors=['#ff4d5e','#4d8dff','#ffd45b','#ff9a3d','#66dd88','#b66dff'];
  const angles=[-90,-30,30,90,150,210];
  colors.forEach((color,i)=>{
    const el=document.createElement('div');
    el.className='sister-signature-bloom';
    el.style.left=`${p.x}px`; el.style.top=`${p.y}px`;
    el.style.setProperty('--sig-color', color);
    const rad=angles[i]*Math.PI/180;
    el.style.setProperty('--dx', `${Math.cos(rad)*42}px`);
    el.style.setProperty('--dy', `${Math.sin(rad)*42}px`);
    layer.appendChild(el); setTimeout(()=>el.remove(),720);
  });
}
function sisterSignatureSealFx(target){
  const layer=ensureFxLayer(); if(!layer || !target) return;
  const p=getCellCenter(target.r,target.c);
  const el=document.createElement('div');
  el.className='sister-signature-seal';
  el.style.left=`${p.x}px`; el.style.top=`${p.y}px`;
  layer.appendChild(el); setTimeout(()=>el.remove(),820);
}
function sisterSignatureBackCell(u,target){
  if(!u || !target) return null;
  const tries=[];
  if(target.facing && DIRS[target.facing]) tries.push(forwardCellAt(target, oppositeDir(target.facing), 1));
  const away=cardinalDirFromDelta(target.r-u.r, target.c-u.c);
  if(away && DIRS[away]) tries.push(forwardCellAt(target, away, 1));
  for(const cell of tries){
    if(cell && clampCell(cell.r,cell.c)){
      const occ=getUnitAt(cell.r,cell.c);
      if(!occ || occ===u) return cell;
    }
  }
  return sisterNearestAdjacentOpenToTarget(u,target);
}
async function sisterSingleSlashBeat(u, target, {caption='', reverse=false, damageAt=null, moveGhost=true, scale=1.30, shake='normal'}={}){
  if(!u || !target) { if(typeof damageAt==='function') damageAt(); return; }
  const targetCell={r:target.r,c:target.c};
  cameraFocusOnCell(target.r,target.c,{scale,hold:520, immediate:true});
  addTempClassToCells([targetCell], 'highlight-imp', 420);
  if(caption) sisterCinematicCaption(caption, 620);
  const dir = cardinalDirFromDelta(target.r-u.r, target.c-u.c) || u.facing || 'right';
  if(moveGhost) sisterSpawnDashGhost(u, {r:u.r,c:u.c}, targetCell, {duration:330, facing:dir, hideReal:true, linger:100});
  await sleep(105);
  sisterPointSlashFx(targetCell, dir, {flip:reverse});
  if(shake) cameraShake(shake);
  await sleep(95);
  if(typeof damageAt==='function') damageAt();
  await sleep(250);
}
function sisterLineEndOpenCell(u, cells){
  let best=null;
  for(const c of uniqueCells(cells||[])){
    if(!clampCell(c.r,c.c)) continue;
    const occ=getUnitAt(c.r,c.c);
    if(!occ || occ===u) best={r:c.r,c:c.c};
  }
  return best;
}
async function sisterLineSlashBeat(u, cells, dir, {caption='', reverse=false, damageAt=null, moveToEnd=false, shake='normal', cls='highlight-imp'}={}){
  const arr=uniqueCells(cells||[]);
  if(!arr.length){ if(typeof damageAt==='function') damageAt(); return; }
  const center=sisterCanvasCenterCell(arr) || arr[0];
  cameraFocusOnCell(center.r, center.c, {scale:1.30, hold:560, immediate:true});
  addTempClassToCells(arr, cls, 440);
  if(caption) sisterCinematicCaption(caption, 650);
  const start={r:u.r,c:u.c};
  const ghostEnd=reverse ? arr[0] : arr[arr.length-1];
  sisterSpawnDashGhost(u, start, ghostEnd, {duration:360, facing:dir || u.facing, hideReal:true, linger:80});
  await sleep(100);
  sisterSlashLaneFx(arr, dir || u.facing, {angle: sisterLateralAngleForDir(dir || u.facing), lenMul:1.25});
  if(shake) cameraShake(shake);
  await sleep(120);
  if(typeof damageAt==='function') damageAt();
  if(moveToEnd){
    const dest=sisterLineEndOpenCell(u, arr);
    if(dest && (dest.r!==u.r || dest.c!==u.c)){
      await sleep(60);
      await animateUnitTravel(u, dest, '穿透回位', {duration:250, teleport:true, noLongTrail:true, endScale:1.16, after:0});
    }
  }
  await sleep(210);
}
async function cinematicAttackWindup(attacker, cells, opts={}){
  const arr = uniqueCells(cells || []);
  if(attacker && arr.length){
    const center = arr[Math.floor(arr.length/2)];
    cameraFocusOnCell(center.r, center.c, {scale:opts.scale || 1.25, hold:opts.hold || 360});
  }
  await telegraphThenImpact(arr);
  if(opts.shake) cameraShake(opts.shake === 'heavy' ? 'heavy' : 'normal');
}
function sisterPlayers(){ return Object.values(units).filter(u=>u && u.side==='player' && u.hp>0); }
function sisterEnemies(){ return Object.values(units).filter(u=>u && u.side==='enemy' && u.hp>0); }
function sisterCollectTargets(cells, side='player'){
  const seen=new Set(); const out=[];
  for(const c of uniqueCells(cells||[])){
    const t=getUnitAt(c.r,c.c);
    if(t && t.hp>0 && (!side || t.side===side) && !seen.has(t.id)){ seen.add(t.id); out.push(t); }
  }
  return out;
}
function sisterPickTargetInCells(cells){
  const t=sisterCollectTargets(cells,'player');
  return t.length ? t[Math.floor(Math.random()*t.length)] : null;
}
function sisterAllUnitsInSquareAround(center, half=2){
  const cells=[];
  for(let r=center.r-half; r<=center.r+half; r++) for(let c=center.c-half; c<=center.c+half; c++) if(r>=1&&r<=ROWS&&c>=1&&c<=COLS) cells.push({r,c});
  return cells;
}
function sisterCellsOutsideRadiusFrom(target, minCheb=4){
  const cells=[];
  for(let r=1;r<=ROWS;r++){
    for(let c=1;c<=COLS;c++){
      if(!clampCell(r,c) || getUnitAt(r,c)) continue;
      if(Math.max(Math.abs(r-target.r), Math.abs(c-target.c)) >= minCheb) cells.push({r,c});
    }
  }
  return cells;
}
function sisterMoveOtherPlayersAway(target){
  const near = sisterPlayers().filter(p=>p.id!==target.id && Math.max(Math.abs(p.r-target.r), Math.abs(p.c-target.c))<=2);
  const farCells = sisterCellsOutsideRadiusFrom(target, 4);
  for(const p of near){
    if(!farCells.length) break;
    const idx=Math.floor(Math.random()*farCells.length);
    const dest=farCells.splice(idx,1)[0];
    sisterTeleportUnit(p,dest,'被剧本推离');
  }
}

async function sisterMoveOtherPlayersAwayAnimated(target){
  const near = sisterPlayers().filter(p=>p.id!==target.id && Math.max(Math.abs(p.r-target.r), Math.abs(p.c-target.c))<=2);
  const farCells = sisterCellsOutsideRadiusFrom(target, 4);
  for(const p of near){
    if(!farCells.length) break;
    const idx=Math.floor(Math.random()*farCells.length);
    const dest=farCells.splice(idx,1)[0];
    await sisterTeleportUnitAnimated(p,dest,'被剧本推离',{duration:520, endScale:1.16});
  }
}
function sisterDetonateBleed(target, source){
  if(!target || !target.status) return 0;
  normalizeBleedState(target);
  const layers = target.status.bleed || 0;
  const strength = target.status.bleedStrength || layers;
  if(layers<=0 || strength<=0) return 0;
  const dmg = Math.max(0, strength * 5);
  target.status.bleed = 0; target.status.bleedStrength = 0; target.status.bleedStrengthQueue = [];
  damageUnit(target.id, dmg, 0, `${source.name} 引爆 ${target.name} 的全部流血（${layers}层）`, source.id, {trueDamage:true, ignoreSisterColorRetaliation:true, sisterFinalStage:true});
  return dmg;
}
function sisterDetonateBlack(target, source){
  const colors=ensureSisterColors(target); if(!colors) return 0;
  const layers = colors.black.layers;
  const strength = colors.black.strength;
  if(layers<=0) return 0;
  const dmg = Math.max(0, layers * 5 + strength * 5);
  colors.black.layers = 0; colors.black.strength = 0;
  damageUnit(target.id, dmg, 0, `${source.name} 引爆 ${target.name} 的黑色层数`, source.id, {trueDamage:true, ignoreSisterColorRetaliation:true, sisterFinalStage:true});
  return dmg;
}
function checkSisterPhase(){
  const s=units && units['sister'];
  if(!s || s.hp<=0 || s._sisterAwakened || s.hp>SISTER_PHASE_HP) return;
  s._sisterAwakened = true;
  s.skillPool.length = 0;
  s.spFloor = SISTER_SP_CRASH_FLOOR;
  s.maxSp = 0;
  ...
  for(const u of Object.values(units)){
    ...
    for(const key of ['red','blue','yellow','orange','green','purple']){
      layers += colors[key].layers;
      strength += colors[key].strength;
      colors[key].layers = 0;
      colors[key].strength = 0;
    }
    if(layers>0) sisterAddColor(u,'black',layers,Math.max(1,Math.round(strength/Math.max(1,layers))),'意识到了转换');
  }
  drawSkills(s, START_HAND_COUNT);
  cameraFocusOnCell(s.r,s.c,{scale:1.35,hold:720});
  cameraShake('heavy');
  renderAll();
}
function checkSisterSpCrash(u){
  if(!u || u.id!=='sister' || u.hp<=0) return;
  // 文档要求：西斯特不是 SP 到 0 就理智崩溃，而是 SP 到 -45 才自伤、失控、-1步。
  // 0 以下只是进入负SP区间；二阶段还会长期保持负SP。
  if(u._sisterSpCrashLock) return;
  if(u.sp <= SISTER_SP_CRASH_FLOOR){
    u._sisterSpCrashLock = true;
    if(!u._spCrashVuln){
      u._spCrashVuln = true;
      appendLog(`${u.name} SP 降至 -45：眩晕期间受到伤害 x1.5`);
    }
    appendLog(`${u.name} SP 降至 -45：自损20真实伤害，失控1回合并 -1 步。`);
    damageUnit(u.id, 20, 0, `${u.name} SP崩溃自伤`, u.id, {trueDamage:true, ignoreSisterColorRetaliation:true, ignoreToughBody:true});
    updateStatusStacks(u,'stunned', Math.max(1,(u.status.stunned||0)+1), {label:'眩晕',type:'debuff'});
    if(u.side==='enemy') enemySteps = Math.max(0, enemySteps - 1); else playerSteps = Math.max(0, playerSteps - 1);
    u.sp = u._sisterAwakened ? -1 : 75;
    updateStepsUI();
    u._sisterSpCrashLock = false;
  }
}
function sisterProcessTurnStart(u, side){
  ...
  if(black.layers>0){
    ...
    if(hpDmg>0) damageUnit(u.id, hpDmg, 0, `${u.name} 的黑色状态造成真实伤害`, null, {trueDamage:true, ignoreSisterColorRetaliation:true});
  }
  if(u.status && u.status.inspiration>0){
    if(side==='enemy') enemySteps += 1; else playerSteps += 1;
    updateStatusStacks(u,'inspiration', Math.max(0,u.status.inspiration-1), {label:'灵感',type:'buff'});
    appendLog(`${u.name} 的灵感：本回合额外 +1 步，灵感 -1`);
    updateStepsUI();
  }
  if(u.id==='sister'){
    if(!u._sisterAwakened && u.hp>SISTER_PHASE_HP){
      sisterAddPaint(u,1,'布网');
      const before=u.sp;
      u.sp = Math.min(u.maxSp, u.sp + 5);
      if(u.sp>before) showGainFloat(u,0,u.sp-before);
      if((u.status.paint||0)>25){
        updateStatusStacks(u,'inspiration', (u.status.inspiration||0)+1, {label:'灵感',type:'buff'});
        appendLog(`${u.name} 的“灵感”触发：颜料超过25，获得1层灵感。`);
      }
    }
    if(u._sisterAwakened){
      const before = enemySteps;
      enemySteps = Math.max(0, enemySteps - 1);
      if(u.sp <= SISTER_SP_DEFENSE_THRESHOLD) enemySteps = Math.max(0, enemySteps - 1);
      if(before!==enemySteps) appendLog(`意识到了：西斯特本回合步数减少（${before}→${enemySteps}）`);
      updateStepsUI();
    }
  }
}
function sisterOnUnitMoved(u){
  if(!u || !u.status) return;
  if(sisterHasAnyColor(u)){
    sisterPaintMarks.add(`${u.r},${u.c}`);
  }
  const s=units && units['sister'];
  if(s && s.hp>0 && s.r===u.r && s.c===u.c && sisterPaintMarks.has(`${u.r},${u.c}`)){
    sisterPaintMarks.delete(`${u.r},${u.c}`);
    sisterAddPaint(s,1,'踩到颜料印记');
  }
}
function sisterApplyOffensiveModifiers(source, target, hpDmg, spDmg, opts){
  let hp = Math.max(0, Number(hpDmg)||0);
  let sp = Math.max(0, Number(spDmg)||0);
  if(!source || !target || source.side===target.side) return {hpDmg:hp, spDmg:sp};
  if(source.status){
    if(source.status.inspiration>0) hp = Math.round(hp * 1.25);
    const completed = source.status.completedWorks || 0;
    if(completed>0){
      const mul = 1 + completed * 0.10;
      hp = Math.round(hp * mul);
      sp = Math.round(sp * mul);
      if(opts && opts.sisterFinalStage){
        opts._sisterCompletedTrueBonus = (opts._sisterCompletedTrueBonus || 0) + completed * 2;
      }
    }
  ...
  if(yellow.layers>0 && (hp>0 || sp>0)){
    ...
    hp = Math.round(hp * (1 - cut));
    sp = Math.round(sp * (1 - cut));
    appendLog(`${source.name} 的黄色状态触发：本次造成伤害降低 ${Math.round(cut*100)}%`);
  }
...
function sisterBlackConversion(target, hpDmg, trueDamage){
  ...
  const pct = Math.min(0.95, 0.10 + strength * 0.05);
  const converted = Math.max(0, Math.round(hpDmg * pct));
  const extra = strength * 5;
  appendLog(`${target.name} 的黑色状态：${converted} 伤害转为真实伤害，并额外 ${extra} 真实伤害`);
  return {hpDmg: Math.max(0, hpDmg - converted), extraTrueHp: converted + extra};
}
function sisterAfterDamageResolved(target, source, finalHp, finalSp, opts={}){
  ...
  if(sisterCanReceiveColorState(target) && (finalHp>0 || finalSp>0)){
    ...
    if(red.layers>0){
      ...
      const dmg = spend.strength * 5;
      if(dmg>0) damageUnit(source.id,dmg,0,`${target.name} 的红色状态反击 ${source.name}`,target.id,{trueDamage:true,ignoreSisterColorRetaliation:true});
    }
    const green=sisterColor(target,'green');
    if(green.layers>0){
      const spend=sisterConsumeColor(target,'green',1);
      const heal = 5 + spend.strength * 5;
      const before=source.hp;
      source.hp=Math.min(source.maxHp, source.hp+heal);
      showGainFloat(source, source.hp-before, 0);
      appendLog(`${target.name} 的绿色状态触发：${source.name} 恢复 ${source.hp-before} HP`);
    }
    const purple=sisterColor(target,'purple');
    if(purple.layers>0){
      const spend=sisterConsumeColor(target,'purple',1);
      const spDmg = spend.strength * 5;
      if(spDmg>0) damageUnit(source.id,0,spDmg,`${target.name} 的紫色状态反击 ${source.name}`,target.id,{trueDamage:true,ignoreSisterColorRetaliation:true});
    }
  }
  if(source.id==='sister' && (finalHp>0 || finalSp>0)){
    addBleed(target,1,1); // 如此锋利
  ...
  if(target.id==='sister' && (finalHp>0 || finalSp>0) && source.side==='player'){
    target._sisterSkillHitsTaken = (target._sisterSkillHitsTaken||0) + 1;
    if(target._sisterAwakened && target._sisterSkillHitsTaken >= 3){
      target._sisterSkillHitsTaken = 0;
      const dest=sisterRandomEmptyCell();
  ...
  if(orange.layers>0 && finalHp>0){
    ...
    const selfDmg = Math.max(1, Math.round(finalHp * reflectPct));
    damageUnit(source.id,selfDmg,0,`${source.name} 的橙色状态反伤`,target.id,{trueDamage:true,ignoreSisterColorRetaliation:true});
  }
}
...
async function playIntroCinematic(){
  ...
  // 进入关卡后立刻尝试播放一阶段 BGM。
  // 如果浏览器因 autoplay 策略阻止声音，后续第一次点击剧情框/页面时会自动重试，不再用“开始”遮罩挡住关卡。
  playSisterBGMPhase(1, {restart:false});
  await sleep(260);
  const boss = units['sister'];
  if(boss && boss.hp>0){
  ...
  await showIntroLine('西斯特：......');
  await showIntroLine('西斯特：你不应该在这里......');
  hideIntroDialog();
  cameraReset();
  await sleep(520);
...
function getSisterBGMForPhase(phase){
  return phase === 2 ? sisterBGMPhase2 : sisterBGMPhase1;
}
function fadeAudio(audio, from, to, ms=600){
  ...
  return new Promise(resolve=>{
    const timer = setInterval(()=>{
      i++;
      const t = i / steps;
      audio.volume = clampValue(from + (to - from) * t, 0, 1);
      if(i >= steps){
        clearInterval(timer);
        audio.volume = to;
        resolve();
...
async function playSisterBGMPhase(phase, {restart=false, fadeMs=0}={}){
  const next = getSisterBGMForPhase(phase);
  if(!next) return false;
  if(sisterCurrentBGM === next && !restart){
    if(next.paused){
      try { await next.play(); } catch(err){ console.log('BGM play error:', err); requestSisterBGMUnlock(phase); return false; }
    }
    return true;
  }
  const prev = sisterCurrentBGM;
  sisterCurrentBGM = next;
  sisterCurrentBGMPhase = phase;
  try {
    if(restart) next.currentTime = 0;
    next.volume = fadeMs ? 0 : 0.72;
    await next.play();
    if(prev && prev !== next){
      if(fadeMs){
        fadeAudio(prev, prev.volume || 0.72, 0, fadeMs).then(()=>{ prev.pause(); prev.currentTime = 0; });
        await fadeAudio(next, 0, 0.72, fadeMs);
      } else {
    ...
    appendLog(phase === 2 ? '🎵 西斯特二阶段 BGM：Heh.... Within Expectations' : '🎵 西斯特一阶段 BGM：Such exquisite grace, yet draped in utter futility.');
    return true;
  } catch (err) {
    console.log('BGM play error:', err);
    requestSisterBGMUnlock(phase);
    return false;
  }
}
function requestSisterBGMUnlock(phase){
  sisterPendingBGMPhase = phase || sisterPendingBGMPhase || 1;
  if(logEl) appendLog('⚠️ 浏览器阻止了自动播放；下一次点击页面/剧情框时会自动开启音乐。');
  if(!sisterMusicUnlockTip){
    sisterMusicUnlockTip = document.createElement('button');
    sisterMusicUnlockTip.id = 'musicUnlockTip';
    sisterMusicUnlockTip.textContent = '开启音乐';
    sisterMusicUnlockTip.title = '浏览器阻止自动播放时，点这里恢复西斯特BGM';
    sisterMusicUnlockTip.addEventListener('click', (e)=>{
      e.stopPropagation();
      retrySisterPendingBGM();
    });
    document.body.appendChild(sisterMusicUnlockTip);
  }
  document.addEventListener('pointerdown', retrySisterPendingBGM, {once:true, capture:true});
  document.addEventListener('keydown', retrySisterPendingBGM, {once:true, capture:true});
}
async function retrySisterPendingBGM(){
  if(!sisterPendingBGMPhase) return;
  const phase = sisterPendingBGMPhase;
  sisterPendingBGMPhase = 0;
  if(sisterMusicUnlockTip){
    sisterMusicUnlockTip.remove();
    sisterMusicUnlockTip = null;
  }
  await playSisterBGMPhase(phase, {restart:false});
}
function stop西斯特BGM() {
  for(const audio of [sisterBGMPhase1, sisterBGMPhase2]){
    if(audio){
      audio.pause();
      audio.currentTime = 0;
    }
  }
  sisterCurrentBGM = null;
  sisterCurrentBGMPhase = 0;
  appendLog('🎵 西斯特主题曲停止播放');
}
...
function apply西斯特DesignPenalty(){
...
function applySpDamage(targetOrId, amount, {sourceId=null, reason=null}={}){
  ...
  if(delta>0){
    ...
    check西斯特Fatigue(u);
    if(u.id==='sister') checkSisterSpCrash(u);
    checkSisterPhase();
    renderAll();
  }
  return delta;
...
function damageUnit(id, hpDmg, spDmg, reason, sourceId=null, opts={}){
  ...
  if(source){
    if(source.side === u.side && source !== u){ appendLog(`友伤无效：${source.name} -> ${u.name}`); return; }
  ...
  if(source && !opts.ignoreSisterOffense){
    const sisterMod = sisterApplyOffensiveModifiers(source, u, hpDmg, spDmg, opts);
    hpDmg = sisterMod.hpDmg;
    spDmg = sisterMod.spDmg;
  }
  ...
  if(!trueDamage && finalHp > 0){
    const blackConv = sisterBlackConversion(u, finalHp, trueDamage);
    finalHp = blackConv.hpDmg;
    sisterExtraTrueHp += blackConv.extraTrueHp || 0;
  }
  if(!trueDamage && finalHp > 0){
  ...
  if(sisterExtraTrueHp > 0) finalHp += sisterExtraTrueHp;
  if(opts._sisterCompletedTrueBonus) finalHp += Math.max(0, Math.round(opts._sisterCompletedTrueBonus));
  ...
  if(u.id==='sister' && u._sisterAwakened && u.sp <= SISTER_SP_DEFENSE_THRESHOLD && u.sp > SISTER_SP_CRASH_FLOOR && finalSp > 0){
    finalSp = Math.max(1, Math.ceil(finalSp * 0.10));
    appendLog(`${u.name} 的“意识到了”：SP防御90%，本次SP伤害降至 ${finalSp}`);
  }

  if(u._godHpLocked && u.side==='player' && u.hp>0 && finalHp>0){
    const allowed = Math.max(0, u.hp - 1);
    if(finalHp > allowed){
      const prevented = finalHp - allowed;
      finalHp = allowed;
      showStatusFloat(u,'锁血',{type:'buff', offsetY:-88});
      appendLog(`GOD’S WILL 锁血：${u.name} 至少保留 1 HP，抵消 ${prevented} HP 伤害`);
    }
  }
  ...
  if(!opts.noShake) cameraShake(heavyHit ? 'heavy' : 'normal');
  ...
  if(!opts.noFloat) showDamageFloat(u, finalHp, finalSp);
  if(!opts.noPulse) pulseCell(u.r, u.c);
  ...
  sisterAfterDamageResolved(u, source, finalHp, finalSp, opts);
  handleSpCrashIfNeeded(u);
  if(u.id==='sister') checkSisterSpCrash(u);
  checkSisterPhase();
...
async function adoraPanicMove(u, payload){
  ...
  await animateUnitTravel(u, dest, '慌乱位移', {duration:620, color:'rgba(92,170,255,0.92)', endScale:1.22});
  registerUnitMove(u);
  showSkillFx('adora:呀！你不要靠近我呀！！',{target:u});
  let chased=false;
  for(const d of Object.keys(DIRS)){
    ...
    if(t && t.side!==u.side && t.hp>0 && t.hp <= t.maxHp/2){ appendLog(`${u.name} 追击残血！`); await adoraDagger(u,t); chased=true; break; }
  }
  if(!chased) unitActed(u);
}
async function adoraZap(u,target){
  if(!target || target.side===u.side){ appendLog('电击装置 目标无效'); return; }
  setUnitFacing(u, cardinalDirFromDelta(target.r-u.r, target.c-u.c));
  await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.25, shake:'normal'});
...
async function darioClaw(u,target){
...
async function darioSwiftMove(u, payload){
  const dest = payload && payload.moveTo; if(!dest){ appendLog('无效的目的地'); return; }
  if(dest.r !== u.r || dest.c !== u.c){
    const dir = cardinalDirFromDelta(dest.r - u.r, dest.c - u.c);
    setUnitFacing(u, dir);
  ...
  await animateUnitTravel(u, dest, '迅捷步伐', {duration:560, color:'rgba(97,190,255,0.92)', endScale:1.20});
  registerUnitMove(u);
  ...
  if(enemies.length){
    ...
    await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.22});
...
async function darioPull(u, targetOrDesc){
  ...
  setUnitFacing(u, usedDir || cardinalDirFromDelta(target.r-u.r,target.c-u.c));
  cameraFocusOnCell(target.r, target.c, {scale:1.20, hold:260});
  showSkillFx('dario:拿来吧你！',{target});
  await sleep(220);
  if(target.pullImmune){ appendLog(`${target.name} 免疫拉扯（小Boss/Boss），改为冲击效果`); }
    ...
    if(placement){
      await animateUnitTravel(target, placement, '被拉到面前', {duration:620, color:'rgba(120,190,255,0.95)', thickness:7, endScale:1.24, shake:'normal'});
    } else {
      appendLog('前方无空位，改为直接造成冲击效果');
    }
  }
  await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.30, shake:'normal'});
...
function darioStatusRecovery(u, aim){
  ...
  if(t.status.bleed > 0){ clearedEffects.push('流血'); setBleedStacksDirect(t, 0); }
...
async function karmaObeyMove(u, payload){
  const dest = payload && payload.moveTo; if(!dest){ appendLog('无效的目的地'); return; }
  if(dest.r !== u.r || dest.c !== u.c){
    const dir = cardinalDirFromDelta(dest.r - u.r, dest.c - u.c);
    setUnitFacing(u, dir);
  ...
  await animateUnitTravel(u, dest, '都听你的', {duration:520, color:'rgba(255,96,96,0.88)', endScale:1.18});
  registerUnitMove(u);
...
async function karmaGrip(u,target){
  if(!target || target.side===u.side){ appendLog('嗜血之握 目标无效'); return; }
  setUnitFacing(u, cardinalDirFromDelta(target.r-u.r, target.c-u.c));
  await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.35, shake:'heavy'});
  let fixed = null;
  // Boss and Elite fixed damage
  if(target.id==='sister') fixed = 75;
...
function mark西斯特SkillUsed(u){
  if(!u) return;
  // Mark that 西斯特 has used a skill this turn (for movement restriction)
  if(u.id === 'khathia' || u.id === 'sister'){
...
async function karmaPunch(u,target){
  ...
  if(adrenalineSkill && u.consecAttacks >= 2 && u.consecAttacks % 2 === 0){
    ...
    setTimeout(async () => {
      if(target.hp > 0){
        await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.20});
...
// —— 西斯特 Boss 技能（文档版第一批实现） ——
function sisterForwardCells(u, dir, depth=2, width=1){
  if(width<=1) return range_forward_n(u, depth, dir);
  return forwardRectCentered(u, dir || u.facing, width, depth).map(c=>({...c,dir:dir||u.facing}));
}
function sisterTargetsInCells(cells){ return sisterCollectTargets(cells, 'player'); }
function sisterPickRandomPlayerInCells(cells){ const targets=sisterTargetsInCells(cells); return targets.length ? targets[Math.floor(Math.random()*targets.length)] : null; }
function sisterPickFrontTarget(u, dir, depth=2, width=1){ return sisterPickRandomPlayerInCells(sisterForwardCells(u,dir,depth,width)); }
function sisterAllDirectionsCells(u, depth=2, width=1){ const out=[]; for(const d of Object.keys(DIRS)) sisterForwardCells(u,d,depth,width).forEach(c=>out.push(c)); return out; }
function sisterNearestAdjacentOpenToTarget(source, target){
  let best=null, bestD=1e9;
  for(const d of Object.keys(DIRS)){
    const cell=forwardCellAt(target,d,1);
    if(!cell || !clampCell(cell.r,cell.c)) continue;
    const occ=getUnitAt(cell.r,cell.c);
    if(occ && occ!==source) continue;
    const dist=mdist(source, cell);
    if(dist<bestD){ bestD=dist; best=cell; }
  }
  return best || sisterRandomOpenCellAround(target,1) || sisterRandomEmptyCell();
}
async function sisterCasualArt(u, payload){
  if(!sisterSpendPaint(u,1)){ unitActed(u); return; }
  const dir = (payload && payload.dir) || (payload && payload.id ? cardinalDirFromDelta(payload.r-u.r,payload.c-u.c) : u.facing);
  setUnitFacing(u, dir);
  const cells = sisterForwardCells(u,dir,2,1);
  await telegraphThenImpact(cells);
  const roll = Math.floor(Math.random()*3);
  let target = payload && payload.id && payload.side==='player' ? payload : sisterPickRandomPlayerInCells(cells);
  if(!target){ appendLog(`${u.name} 随性作画：没有命中目标`); unitActed(u); return; }
  if(roll===0){
    damageUnit(target.id,7,5,`${u.name} 随性作画·红怨 第一笔 ${target.name}`,u.id,{skillFx:'sister:随性作画'});
    damageUnit(target.id,10,0,`${u.name} 随性作画·红怨 第二笔 ${target.name}`,u.id,{skillFx:'sister:随性作画',sisterFinalStage:true});
    sisterAddColor(target,'red',2,1,'红怨');
  }else if(roll===1){
    damageUnit(target.id,0,15,`${u.name} 随性作画·蓝调 刺入 ${target.name}`,u.id,{skillFx:'sister:随性作画',sisterFinalStage:true});
    sisterAddColor(target,'blue',1,1,'蓝调');
  }else{
    damageUnit(target.id,10,0,`${u.name} 随性作画·黄堕 涂抹 ${target.name}`,u.id,{skillFx:'sister:随性作画',sisterFinalStage:true});
    sisterAddColor(target,'yellow',2,1,'黄堕');
  }
  u.dmgDone += 10;
  unitActed(u);
}
async function sisterScriptWriting(u, aim){
  const target = getUnitAt(aim.r, aim.c);
  if(!target || target.side!=='player'){ appendLog('剧本创作 目标无效'); unitActed(u); return; }
  const dest = sisterNearestAdjacentOpenToTarget(u,target);
  if(dest) await sisterTeleportUnitAnimated(u,dest,'剧本创作·入场',{duration:240, endScale:1.20});
  setUnitFacing(u, cardinalDirFromDelta(target.r-u.r,target.c-u.c));
  await sisterMoveOtherPlayersAwayAnimated(target);
  await sisterSingleSlashBeat(u, target, {
    caption:'剧本创作', shake:'normal',
    damageAt:()=>{
      damageUnit(target.id,5,0,`${u.name} 剧本创作 斩向 ${target.name}`,u.id,{skillFx:'sister:剧本创作',sisterFinalStage:true});
      addBleed(target,1,1);
      u.dmgDone += 5;
    }
  });
  u._sisterScriptUses = (u._sisterScriptUses||0) + 1;
  u._sisterScriptUsesThisTurn = (u._sisterScriptUsesThisTurn||0) + 1;
  unitActed(u);
}

async function sisterCanvas(u){
  const targets=sisterPlayers();
  if(!targets.length){ unitActed(u); return; }
  const target=targets[Math.floor(Math.random()*targets.length)];
  const dest=sisterRandomOpenCellAround(target,2) || sisterRandomEmptyCell();
  if(dest) await sisterTeleportUnitAnimated(u,dest,'这里是我的画布',{duration:360, endScale:1.22});
  unitActed(u);
}

function sisterOutOfPaint(u){
  for(let i=0;i<4;i++) sisterAddPaint(u,1,'啧，又空了');
  let total=0;
  for(const v of Object.values(units)){ if(v && v.hp>0) total += sisterTotalColorLayers(v,true); }
  if(total>0) sisterAddPaint(u,total,'扫描全场特殊颜色状态');
  unitActed(u);
}
async function sisterSignature(u, aim){
  let target = getUnitAt(aim.r, aim.c);
  if(!target || !sisterEligibleForSignature(target)){ const list=sisterSignatureEligibleTargets(); target=list[0]; }
  if(!target){ appendLog('签名：没有符合条件的目标'); unitActed(u); return; }

  // 签名演出：绕到背后 → 六色收束 → 压低SP → 前斩/回斩 → 黑墨落款。
  const dest = sisterSignatureBackCell(u,target);
  if(dest) await sisterTeleportUnitAnimated(u,dest,'签名·绕后',{duration:260, endScale:1.20});
  setUnitFacing(u, cardinalDirFromDelta(target.r-u.r,target.c-u.c));
  sisterFocusCells([{r:u.r,c:u.c},{r:target.r,c:target.c}], {scale:1.34, hold:520, caption:'签名'});
  await sleep(110);

  sisterSignatureBloomFx(target);
  sisterCinematicCaption('六色收束', 620);
  await sleep(190);
  for(const key of ['red','blue','yellow','orange','green','purple']) sisterAddColor(target,key,1,1,'签名补色');
  renderAll();

  await sisterSingleSlashBeat(u, target, {caption:'压低', moveGhost:false, shake:'normal', damageAt:()=>{
    damageUnit(target.id,0,10,`${u.name} 签名·压低 ${target.name} 的SP`,u.id,{skillFx:'sister:签名'});
  }});
  await sleep(70);

  await sisterSingleSlashBeat(u, target, {caption:'前斩', shake:'normal', damageAt:()=>{
    damageUnit(target.id,10,0,`${u.name} 签名·前斩 ${target.name}`,u.id,{skillFx:'sister:签名'});
  }});
  await sleep(80);

  await sisterSingleSlashBeat(u, target, {caption:'回斩', reverse:true, shake:'normal', damageAt:()=>{
    damageUnit(target.id,5,0,`${u.name} 签名·回斩 ${target.name}`,u.id,{skillFx:'sister:签名'});
  }});
  await sleep(90);

  sisterSignatureSealFx(target);
  sisterScreenFlash(420);
  await sisterSingleSlashBeat(u, target, {caption:'落款', moveGhost:false, shake:'heavy', damageAt:()=>{
    damageUnit(target.id,25,0,`${u.name} 用黑色墨水签上名字`,u.id,{trueDamage:true,skillFx:'sister:签名',sisterFinalStage:true});
  }});
  sisterAddColor(target,'black',3,2,'签名');
  updateStatusStacks(u,'completedWorks',(u.status.completedWorks||0)+1,{label:'完成的作品',type:'buff'});
  u._sisterSignedTargets[target.id]=true;
  u.dmgDone += 40;
  unitActed(u);
}


async function sisterVentArt(u, payload){
  if(!sisterSpendPaint(u,1)){ unitActed(u); return; }
  const dir = (payload && payload.dir) || (payload && payload.id ? cardinalDirFromDelta(payload.r-u.r,payload.c-u.c) : u.facing);
  setUnitFacing(u,dir);
  const roll=Math.floor(Math.random()*3);
  if(roll===0){
    const cells=sisterForwardCells(u,dir,3,1);
    const targets=sisterTargetsInCells(cells);
    let far=null, farD=-1;
    await sisterLineSlashBeat(u, cells, dir, {caption:'怨·染黑', shake:'normal', damageAt:()=>{
      for(const t of targets){
        damageUnit(t.id,20,0,`${u.name} 发泄性作画·怨 染黑 ${t.name}`,u.id,{skillFx:'sister:发泄性作画'});
        sisterAddColor(t,'black',1,1,'怨');
        const d=mdist(u,t); if(d>farD){ farD=d; far=t; }
      }
    }});
    if(far && (far.status.bleed||0)>=3){
      const dest=sisterNearestAdjacentOpenToTarget(u,far);
      if(dest) await sisterTeleportUnitAnimated(u,dest,'怨·入背',{duration:220, endScale:1.20});
      setUnitFacing(u, cardinalDirFromDelta(far.r-u.r, far.c-u.c));
      await sisterSingleSlashBeat(u, far, {caption:'砍下', shake:'normal', damageAt:()=>{
        damageUnit(far.id,5,0,`${u.name} 怨·砍下 ${far.name}`,u.id,{skillFx:'sister:发泄性作画'});
      }});
      await sisterSingleSlashBeat(u, far, {caption:'再砍', reverse:true, shake:'normal', damageAt:()=>{
        damageUnit(far.id,5,0,`${u.name} 怨·再次砍下 ${far.name}`,u.id,{skillFx:'sister:发泄性作画'});
      }});
      const backDir=oppositeDir(u.facing);
      const backCells=sisterForwardCells(u, backDir, 3, 1);
      await sisterLineSlashBeat(u, backCells, backDir, {caption:'穿透回位', shake:'heavy', moveToEnd:true, damageAt:()=>{
        for(const t of sisterTargetsInCells(backCells)) damageUnit(t.id,15,0,`${u.name} 怨·穿透回位 ${t.name}`,u.id,{skillFx:'sister:发泄性作画',sisterFinalStage:true});
        for(const t of targets) sisterDetonateBleed(t,u);
      }});
    }
  }else if(roll===1){
    const cells=sisterForwardCells(u,dir,2,3);
    await cinematicAttackWindup(u, cells, {scale:1.34, shake:'normal'});
    const targets=sisterTargetsInCells(cells);
    for(const t of targets){
      damageUnit(t.id,5,15,`${u.name} 发泄性作画·悲 淹没 ${t.name}`,u.id,{skillFx:'sister:发泄性作画',sisterFinalStage:true});
      sisterAddColor(t,'black',1,1,'悲');
    }
    let blackNear=0;
    for(const t of sisterPlayers()) if(Math.max(Math.abs(t.r-u.r),Math.abs(t.c-u.c))<=3) blackNear += sisterColor(t,'black').layers;
    if(blackNear>0){ sisterAddPermanentDamageBoost(u, blackNear, `的“悲”：根据7x7黑色层数获得增伤`); }
  }else{
    const cells=sisterForwardCells(u,dir,2,1);
    await cinematicAttackWindup(u, cells, {scale:1.32, shake:'normal'});
    let target=sisterPickRandomPlayerInCells(cells);
    if(!target){ appendLog('奈：前方两格没有目标'); unitActed(u); return; }
    for(let i=1;i<=6;i++){
      await sleep(i===1 ? 30 : 210);
      sisterPointSlashFx({r:target.r,c:target.c}, dir, {flip:i%2===0});
      if(i===1 || i===6) cameraFocusOnCell(target.r,target.c,{scale:1.28,hold:360, immediate:true});
      await sleep(75);
      damageUnit(target.id,5,0,`${u.name} 发泄性作画·奈 第${i}刀 ${target.name}`,u.id,{skillFx:null,sisterFinalStage:i===6,noFx:true,noShake:i!==6,noPulse:true});
      const heal=Math.ceil(5*0.5); const before=u.hp; u.hp=Math.min(u.maxHp,u.hp+heal); if(u.hp>before) showGainFloat(u,u.hp-before,0);
      if(i===1) sisterAddColor(target,'black',1,1,'奈');
      if(target.hp<=0) break;
    }
    if(target.hp>0 && (target.status.bleed||0)>=10){
      await cinematicAttackWindup(u, [{r:target.r,c:target.c}], {scale:1.32, shake:'normal'});
      damageUnit(target.id,10,0,`${u.name} 奈·流血过载追加 ${target.name}`,u.id,{skillFx:'sister:发泄性作画',sisterFinalStage:true});
      const before=u.hp; u.hp=Math.min(u.maxHp,u.hp+5); if(u.hp>before) showGainFloat(u,u.hp-before,0);
    }
  }
  unitActed(u);
}
...
function sisterLeftVec(dir){
  switch(dir){
    case 'up': return {dr:0, dc:-1};
    case 'down': return {dr:0, dc:1};
    case 'left': return {dr:1, dc:0};
    case 'right': return {dr:-1, dc:0};
    default: return {dr:0, dc:-1};
  }
}
function sisterForwardCanvasCells(u, dir, {lateralMin=-2, lateralMax=2, stepMin=1, stepMax=5}={}){
  const d = DIRS[dir] || DIRS[u.facing] || DIRS.right;
  const left = sisterLeftVec(dir || u.facing);
  const out=[];
  for(let step=stepMin; step<=stepMax; step++){
    for(let lat=lateralMin; lat<=lateralMax; lat++){
      const r = u.r + d.dr*step + left.dr*lat;
      const c = u.c + d.dc*step + left.dc*lat;
      if(clampCell(r,c)) out.push({r,c,dir:dir||u.facing, step, lat});
    }
  }
  return uniqueCells(out);
}
function sisterCanvasCenterCell(cells){
  const arr=uniqueCells(cells||[]);
  if(!arr.length) return null;
  const r=Math.round(arr.reduce((sum,p)=>sum+p.r,0)/arr.length);
  const c=Math.round(arr.reduce((sum,p)=>sum+p.c,0)/arr.length);
  return {r:clampValue(r,1,ROWS), c:clampValue(c,1,COLS)};
}
function sisterUnitsInCells(cells, exclude=null){
  const set = new Set(uniqueCells(cells||[]).map(p=>`${p.r},${p.c}`));
  return Object.values(units).filter(t=>t && t.hp>0 && t!==exclude && set.has(`${t.r},${t.c}`));
}
function sisterRandomEmptyCellInCells(cells){
  const arr = uniqueCells(cells||[]).filter(p=>clampCell(p.r,p.c) && !getUnitAt(p.r,p.c));
  return arr.length ? arr[Math.floor(Math.random()*arr.length)] : null;
}
function sisterCinematicCaption(text, ms=980){
  if(!battleAreaEl) return;
  const el = document.createElement('div');
  el.className = 'sister-cinematic-caption';
  el.textContent = text;
  battleAreaEl.appendChild(el);
  setTimeout(()=>el.remove(), ms + 80);
}
function sisterScreenFlash(ms=560){
  if(!battleAreaEl) return;
  const el = document.createElement('div');
  el.className = 'sister-screen-flash';
  battleAreaEl.appendChild(el);
  setTimeout(()=>el.remove(), ms + 80);
}
function sisterFocusCells(cells, {scale=1.38, hold=620, shake=null, caption=null}={}){
  const center = sisterCanvasCenterCell(cells);
  if(center) cameraFocusOnCell(center.r, center.c, {scale, hold});
  if(caption) sisterCinematicCaption(caption);
  if(shake) cameraShake(shake);
}
async function sisterPublishTelegraph(cells, cls, ms=360, opts={}){
  const arr=uniqueCells(cells||[]);
  sisterFocusCells(arr, opts);
  addTempClassToCells(arr, cls, ms);
  await sleep(ms);
}
function sisterShowFxOnCells(skillKey, cells, attacker){
  const arr=uniqueCells(cells||[]);
  for(const p of arr){
    showSkillFx(skillKey, {attacker, cell:p, fxCell:p});
  }
}

function sisterCellsBounds(cells){
  const arr=uniqueCells(cells||[]).map(c=>getCellEl(c.r,c.c)).filter(Boolean);
  if(!arr.length) return null;
  let left=Infinity, top=Infinity, right=-Infinity, bottom=-Infinity;
  for(const el of arr){
    left=Math.min(left, el.offsetLeft); top=Math.min(top, el.offsetTop);
    right=Math.max(right, el.offsetLeft + el.offsetWidth); bottom=Math.max(bottom, el.offsetTop + el.offsetHeight);
  }
  return {left, top, right, bottom, width:right-left, height:bottom-top, centerX:(left+right)/2, centerY:(top+bottom)/2};
}
function sisterLateralAngleForDir(dir){
  const left=sisterLeftVec(dir||'up');
  // 横向是左右扫，纵向是上下扫。加一点角度让斩击不是死板直线。
  if(Math.abs(left.dc) > Math.abs(left.dr)) return left.dc < 0 ? -8 : 172;
  return left.dr < 0 ? -98 : 82;
}
function sisterAreaBlinkAtCell(cell){
  const layer=ensureFxLayer(); if(!layer || !cell) return;
  const p=getCellCenter(cell.r,cell.c);
  const el=document.createElement('div');
  el.className='sister-publish-blink';
  el.style.left=`${p.x}px`; el.style.top=`${p.y}px`;
  layer.appendChild(el); setTimeout(()=>el.remove(),340);
}
async function sisterPublishBlinkMoveBatch(moves, label='传送'){
  const layer=ensureFxLayer();
  const valid=(moves||[]).filter(m=>m && m.u && m.dest && clampCell(m.dest.r,m.dest.c));
  if(!valid.length) return;
  for(const m of valid){ sisterAreaBlinkAtCell({r:m.u.r,c:m.u.c}); }
  await sleep(130);
  for(const m of valid){
    const old={r:m.u.r,c:m.u.c};
    m.u.r=m.dest.r; m.u.c=m.dest.c;
    m._old=old;
  }
  renderAll();
  for(const m of valid){ sisterAreaBlinkAtCell(m.dest); pulseCell(m.dest.r,m.dest.c); }
  if(valid.length===1){ const m=valid[0]; appendLog(`${m.u.name} ${label}：(${m._old.r},${m._old.c}) → (${m.u.r},${m.u.c})`); }
  else appendLog(`${label}：${valid.length} 个单位被重排`);
  await sleep(190);
}
async function sisterPublishSweep(u, cells, dir, {reverse=false, caption='', damageAt=null, shake='normal'}={}){
  const layer=ensureFxLayer();
  const bounds=sisterCellsBounds(cells);
  if(!bounds){ if(typeof damageAt==='function') damageAt(); return; }
  const angle=sisterLateralAngleForDir(dir);
  const el=document.createElement('div');
  el.className=`sister-publish-sweep${reverse?' reverse':''}`;
  el.style.left=`${bounds.left}px`; el.style.top=`${bounds.top}px`;
  el.style.width=`${bounds.width}px`; el.style.height=`${bounds.height}px`;
  el.innerHTML='<div class="cutline"></div><div class="blade"></div>';
  el.style.setProperty('--blade-angle', `${angle}deg`);
  el.style.setProperty('--blade-len', `${Math.max(bounds.width,bounds.height)*1.65}px`);
  if(layer) layer.appendChild(el);

  const axisHorizontal = Math.abs((sisterLeftVec(dir)||{}).dc || 0) > Math.abs((sisterLeftVec(dir)||{}).dr || 0);
  const pad=20;
  const startX = axisHorizontal ? (reverse ? bounds.right + pad : bounds.left - pad) : bounds.centerX;
  const endX   = axisHorizontal ? (reverse ? bounds.left - pad : bounds.right + pad) : bounds.centerX;
  const startY = axisHorizontal ? bounds.centerY : (reverse ? bounds.bottom + pad : bounds.top - pad);
  const endY   = axisHorizontal ? bounds.centerY : (reverse ? bounds.top - pad : bounds.bottom + pad);
  const ghost=document.createElement('div');
  const realEl=document.querySelector(`.unit[data-id="${u.id}"]`) || document.querySelector(`.largeOverlay[data-id="${u.id}"]`);
  const boundsUnit=getUnitBounds(u);
  ghost.className=`unit unit-dash-ghost sister-cine enemy`;
  ghost.dataset.facing = u.facing || dir || 'right';
  ghost.innerHTML = realEl ? realEl.innerHTML : `<div>${u.name}</div>`;
  ghost.style.left=`${startX}px`; ghost.style.top=`${startY}px`;
  ghost.style.right='auto'; ghost.style.bottom='auto';
  ghost.style.width=`${boundsUnit ? boundsUnit.width : 38}px`;
  ghost.style.height=`${boundsUnit ? boundsUnit.height : 38}px`;
  ghost.style.setProperty('--travel-ms', `460ms`);
  if(layer) layer.appendChild(ghost);
  if(realEl) realEl.classList.add('unit-travel-hidden');
  await sleep(40);
  ghost.style.opacity='1';
  ghost.style.transform=`translate(-50%,-50%) translate(${endX-startX}px, ${endY-startY}px) scale(1.08)`;
  if(caption) sisterCinematicCaption(caption, 720);
  if(shake) cameraShake(shake);
  let damaged=false;
  await sleep(230);
  ...
  await sleep(330);
  if(!damaged && typeof damageAt==='function') damageAt();
  ghost.style.opacity='0';
  if(realEl) realEl.classList.remove('unit-travel-hidden');
...
async function sisterPublishAreaImpact(cells, {caption='', damageAt=null, shake='heavy', cls='highlight-publish-slam'}={}){
  const layer=ensureFxLayer();
  const bounds=sisterCellsBounds(cells);
  if(bounds && layer){
    const el=document.createElement('div');
    el.className='sister-area-smash';
    el.style.left=`${bounds.left}px`; el.style.top=`${bounds.top}px`;
    el.style.width=`${bounds.width}px`; el.style.height=`${bounds.height}px`;
    layer.appendChild(el); setTimeout(()=>el.remove(),620);
  }
  if(caption) sisterCinematicCaption(caption, 720);
  addTempClassToCells(cells, cls, 520);
  if(shake) cameraShake(shake);
  await sleep(220);
  if(typeof damageAt==='function') damageAt();
  await sleep(330);
...
async function sisterPublish(u){
  const dir = u.facing || 'up';
  const all5x5 = sisterForwardCanvasCells(u, dir);
  const left2x5 = sisterForwardCanvasCells(u, dir, {lateralMin:1, lateralMax:2, stepMin:1, stepMax:5});
  const right2x5 = sisterForwardCanvasCells(u, dir, {lateralMin:-2, lateralMax:-1, stepMin:1, stepMax:5});
  const center3x3 = sisterForwardCanvasCells(u, dir, {lateralMin:-1, lateralMax:1, stepMin:2, stepMax:4});
  const hitTargets=[];
  const rememberHit = (arr)=>{ for(const t of arr){ if(t && !hitTargets.includes(t)) hitTargets.push(t); } };

  sisterAddPaint(u,1,'发布');
  appendLog(`${u.name} 展开“发布”：黑色累计 ${(u._sisterCumulativeBlack||0)}，剩余步数全部投入。`);

  // 开幕只把镜头带到画布，不再先拉近很久再等伤害。
  sisterFocusCells(all5x5, {scale:1.30, hold:620, caption:'发布'});
  addTempClassToCells(all5x5, 'highlight-publish-zone', 620);
  await sleep(180);

  // (2) 把所有没在自己面前5x5格内的单位拖入画布，并上一层黑色（强度1）。
  // 批量闪入，避免一个一个长距离拖线导致卡顿和画面混乱。
  const canvasSet = new Set(all5x5.map(p=>`${p.r},${p.c}`));
  const allLiving = Object.values(units).filter(t=>t && t.hp>0 && t!==u);
  const moves=[];
  for(const t of allLiving){
    if(!canvasSet.has(`${t.r},${t.c}`)){
      const dest = sisterRandomEmptyCellInCells(all5x5) || sisterRandomEmptyCell();
      if(dest) moves.push({u:t,dest});
    }
  }
  if(moves.length) await sisterPublishBlinkMoveBatch(moves, '发布拖入5x5画布');
  for(const t of allLiving){ sisterAddColor(t,'black',1,1,'发布开幕'); }
  renderAll();
  await sleep(180);
  ...
  for(let i=0; i<publishCuts.length; i++){
    const cut = publishCuts[i];
    addTempClassToCells(cut.cells, cut.cls, 380);
    await sisterPublishSweep(u, cut.cells, dir, {
      reverse:cut.reverse,
      caption:cut.caption,
      shake:'normal',
      damageAt:()=>{
        const targets = sisterUnitsInCells(cut.cells, u).filter(t=>t.side!==u.side);
        for(const t of targets){
  ...
  // (5) 天砸中间3x3：快速坠落，落点同时结算。
  await sisterPublishAreaImpact(center3x3, {
    caption:'中心坠落', cls:'highlight-publish-center', shake:'heavy',
    damageAt:()=>{
      const centerTargets = sisterUnitsInCells(center3x3, u).filter(t=>t.side!==u.side);
  ...
  // (6) 整个5x5再砸一次，并引爆黑色。
  await sisterPublishAreaImpact(all5x5, {
    caption:'整幅压下', cls:'highlight-publish-slam', shake:'heavy',
    damageAt:()=>{
      const slamTargets = sisterUnitsInCells(all5x5, u).filter(t=>t.side!==u.side);
      for(const t of slamTargets){
        damageUnit(t.id,20,0,`${u.name} 发布·整幅砸落 ${t.name}`,u.id,{skillFx:null,ignoreCover:true,sisterBoostToken:'publish-slam',noFx:true,noShake:true,noPulse:true});
      }
      rememberHit(slamTargets);
      for(const t of slamTargets){ sisterDetonateBlack(t,u); }
      renderAll();
    }
  });
  await sleep(140);

  // (7) 砸飞：同样改成批量闪飞，不再一个个跨图拖线。
  const flingTargets = hitTargets.filter(t=>t && t.hp>0 && canvasSet.has(`${t.r},${t.c}`));
  if(flingTargets.length){
    sisterCinematicCaption('散场', 560);
    const flingMoves=[];
    for(const t of flingTargets){
      const dest=sisterRandomEmptyCell();
      if(dest) flingMoves.push({u:t,dest});
    }
    await sisterPublishBlinkMoveBatch(flingMoves, '发布砸飞');
  }
  const stunPool = hitTargets.filter(t=>t && t.hp>0);
  if(stunPool.length){
    const victim=stunPool[Math.floor(Math.random()*stunPool.length)];
    updateStatusStacks(victim,'stunned',(victim.status.stunned||0)+1,{label:'眩晕',type:'debuff'});
    showStatusFloat(victim,'眩晕',{type:'debuff',offsetY:-72});
    appendLog(`发布：${victim.name} 被随机眩晕`);
  }
  cameraShake('heavy');
  setTimeout(()=>cameraReset(), 260);
  unitActed(u);
}
function buildSisterSkillFactories(u){
  ...
  if(publishReady) return [publishFactory];
  if(signatureReady) return [signatureFactory];
  const F=[];
  if(!u._sisterAwakened){
    F.push({ key:'随性作画', prob:0.80, cond:()=> (u.status.paint||0)>0, make:()=> skill('随性作画',1,'pink','消耗1颜料，随机释放红怨/蓝调/黄堕',
      (uu,aimDir)=> aimDir? sisterForwardCells(uu,aimDir,2,1) : sisterAllDirectionsCells(uu,2,1), (uu,p)=>sisterCasualArt(uu,p), {}, {castMs:900}) });
  }else{
    F.push({ key:'发泄性作画', prob:0.80, cond:()=> (u.status.paint||0)>0, make:()=> skill('发泄性作画',2,'red','消耗1颜料，随机释放怨/悲/奈',
      (uu,aimDir)=> aimDir? sisterForwardCells(uu,aimDir,3,3) : sisterAllDirectionsCells(uu,3,3), (uu,p)=>sisterVentArt(uu,p), {aoe:true}, {castMs:1500}) });
  }
  F.push(
    { key:'剧本创作', prob:0.60, cond:()=>true, make:()=> skill('剧本创作',3,'red','5x5内选敌：瞬移到面前，推离周围敌人，造成5HP+1流血',
      (uu)=> range_square_n(uu,2).filter(p=>{ const tu=getUnitAt(p.r,p.c); return tu && tu.side==='player'; }), (uu,aim)=>sisterScriptWriting(uu,aim), {}, {cellTargeting:true, castMs:1200}) },
    { key:'这里是我的画布', prob:0.20, cond:()=>sisterPlayers().length>0, make:()=> skill('这里是我的画布',3,'blue','传送到随机敌方单位周围5x5格内',
      (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}], (uu)=>sisterCanvas(uu), {}, {castMs:800}) },
    { key:'啧，又空了', prob:1, cond:()=> (u.status.paint||0)<2 && !(u.skillPool||[]).some(s=>s && s.name==='啧，又空了'), make:()=> skill('啧，又空了',5,'white','颜料少于2时获得一次；使用后获得4层颜料，并按全场特殊颜色总层数额外获得颜料',
      (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}], (uu)=>sisterOutOfPaint(uu), {}, {extraSkill:true, castMs:900}) },
    signatureFactory,
    publishFactory
  );
  return F;
...
function getSelectedSkillKeysForUnit(u) {
  ...
  for (const color of ['green', 'gray', 'blue', 'pink', 'white', 'red', 'purple']) {
...
function grantBlackFlashRelease(u){
  ...
  const release = skill('黑瞬「释放」', 3, 'purple', '敌方全体受到其最大SP的50%+30SP伤害（不受掩体）',
    ...
    (uu)=>{
      ...
      for(const t of enemies){ const spDmg = Math.floor((t.maxSp || 0) * 0.5) + 30; damageUnit(t.id, 0, spDmg, `${uu.name} 黑瞬「释放」命中 ${t.name}`, uu.id, {ignoreCover:true}); }
...
function buildSkillFactoriesForUnit(u){
  ...
  if(u.id==='adora'){
    F.push(
      ...
      { key:'枪击', prob:0.65, cond:()=>inventory.pistol, make:()=> skill('枪击',1,'gray','指定方向整排 10HP+5SP（需手枪）',
    ...
    F.push(...buildSisterSkillFactories(u));
  ...
  if(u.id==='adora'){
    F.push({ key:'黑瞬「充能」', prob:0.20, cond:()=>u.level>=50 && !hasSkillInPool(u,'黑瞬「释放」'), make:()=> skill('黑瞬「充能」',2,'purple','随机生成3个墨片；友方踩完全部墨片后获得额外技能“黑瞬「释放」”',
      (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}],
...
function drawOneSkill(u){
  // Unified draw: buildSkillFactoriesForUnit already maps selected skill ids to battle keys.
  // Older code compared raw ids (e.g. adora_bloom) against Chinese keys (e.g. 绽放（红色）), causing empty hands.
...
function drawSkills(u, n){
  const maxPool = (u && u.id==='sister') ? SKILLPOOL_MAX + 5 : SKILLPOOL_MAX;
  let toDraw = Math.max(0, Math.min(n, maxPool - u.skillPool.length));
  while(toDraw>0){ const sk=drawOneSkill(u); if(!sk) break; u.skillPool.push(sk); toDraw--; }
  if(u.skillPool.length > maxPool) u.skillPool.length = maxPool;
}
...
function showGodsWillMenuAtUnit(u){
  ...
  if(godsWillMenuEl){ godsWillMenuEl.remove(); godsWillMenuEl=null; }
  const p = getCellScreenCenter(u.r, u.c);
  godsWillMenuEl = document.createElement('div');
  godsWillMenuEl.className = 'gods-menu';
  godsWillMenuEl.style.left = `${Math.max(8, p.x + 8)}px`;
  godsWillMenuEl.style.top  = `${Math.max(8, p.y - 8)}px`;
  const allyLockButtons = u.side === 'player' ? `
      <button class="lockhp">锁血</button>
      <button class="unlockhp">解除锁血</button>` : '';
  godsWillMenuEl.innerHTML = `
    <div class="title">GOD’S WILL → ${u.name}${u._godHpLocked ? '（锁血中）' : ''}</div>
    <div class="row">
      <button class="kill">杀死</button>
      <button class="onehp">留 1 HP</button>${allyLockButtons}
  ...
  const lockBtn = godsWillMenuEl.querySelector('.lockhp');
  if(lockBtn) lockBtn.onclick = (e)=>{
    e.stopPropagation();
    u._godHpLocked = true;
    appendLog(`GOD’S WILL：${u.name} 锁血开启（受到伤害时至少保留 1 HP）`);
    showStatusFloat(u,'锁血',{type:'buff', offsetY:-88});
    renderAll();
    disarmGodsWill();
  };
  const unlockBtn = godsWillMenuEl.querySelector('.unlockhp');
  if(unlockBtn) unlockBtn.onclick = (e)=>{
    e.stopPropagation();
    u._godHpLocked = false;
    appendLog(`GOD’S WILL：${u.name} 锁血解除`);
    showStatusFloat(u,'解除锁血',{type:'debuff', offsetY:-88});
    renderAll();
    disarmGodsWill();
  };
...
function buildGrid(){
  ...
  for(let r=1;r<=ROWS;r++){
    for(let c=1;c<=COLS;c++){
      ...
      if(sisterPaintMarks && sisterPaintMarks.has(`${r},${c}`)) cell.classList.add('paint-mark');
...
function getSpBarDisplay(u){
  // For units with negative SP (like 西斯特 with spFloor=-100):
...
function summarizeNegatives(u){
  ...
  if(typeof ensureSisterColors === 'function'){
    const colors = ensureSisterColors(u);
    const shown = SISTER_COLOR_KEYS.filter(k=>colors && colors[k] && colors[k].layers>0).map(k=>`${SISTER_COLOR_LABELS[k]}x${colors[k].layers}(强${colors[k].strength})`);
    if(shown.length) parts.push(shown.join(' '));
    if(u.status.paint>0) parts.push(`颜料x${u.status.paint}`);
    if(u.status.inspiration>0) parts.push(`灵感x${u.status.inspiration}`);
    if(u.status.completedWorks>0) parts.push(`完成作品x${u.status.completedWorks}`);
    if(u.status.sisterDamageBoost>0) parts.push(`增伤x${u.status.sisterDamageBoost}`);
  }
  if(u._godHpLocked) parts.push('锁血');
...
function renderStatus(){
  ...
  const enemyWrap=document.createElement('div'); enemyWrap.style.marginTop='10px'; enemyWrap.innerHTML='<strong>敌方（西斯特）</strong>';
...
async function handleSkillConfirmCell(u, sk, aimCell){
  ...
  const realCost = (sk.meta && sk.meta.consumeAllSteps) ? currentSteps : sk.cost;
  if(!(sk.meta && sk.meta.consumeAllSteps) && sk.cost > currentSteps){ appendLog('步数不足'); clearSkillAiming(); renderAll(); return; }

  const aimDir = resolveAimDirForSkill(u, sk, aimCell);
  const cells = sk.rangeFn(u, aimDir, aimCell) || [];
  if(!rangeIncludeCell(cells, aimCell)){ appendLog('该格不在技能范围内'); return; }

  if(u.side==='player'){ playerSteps = Math.max(0, playerSteps - realCost); } else { enemySteps = Math.max(0, enemySteps - realCost); }
...
async function onCellClick(r,c){
  ...
  // 西斯特 restriction: cannot move after casting a skill
  ...
  setInteractionLocked(true);
  await animateUnitTravel(sel, {r,c}, '移动', {duration:360, color: sel.side==='player' ? 'rgba(88,153,255,0.80)' : 'rgba(180,80,255,0.80)', endScale:1.12, thickness:5});
  setInteractionLocked(false);
  ...
  if(skillPool){
    if(u.side==='enemy'){ skillPool.innerHTML = `<div class="partyRow small">敌方单位（AI 控制），无法操作</div>`; }
      ...
      for(const sk of pool){
        const stepsOk = (sk.meta && sk.meta.consumeAllSteps) ? playerSteps>0 : playerSteps>=sk.cost;
        ...
        rightBox.textContent = (sk.meta && sk.meta.consumeAllSteps) ? '全部步' : `${sk.cost} 步`;
        ...
        card.addEventListener('click', ()=>{
          ...
          // 西斯特 restriction: cannot cast skill after moving
...
function clearHighlights(){ highlighted.clear(); document.querySelectorAll('.cell').forEach(cell=>cell.classList.remove('highlight-move','highlight-skill','highlight-skill-target','pulse','highlight-tele','highlight-imp','highlight-stage','highlight-path','highlight-publish-zone','highlight-publish-left','highlight-publish-right','highlight-publish-center','highlight-publish-slam')); }
...
function processUnitsTurnStart(side){
  ...
  for(const id in units){
    ...
    if(u.id === 'sister') u._sisterScriptUsesThisTurn = 0;
    ...
    sisterProcessTurnStart(u, side);

    if(u.id==='khathia' && side==='enemy' && u.turnsStarted % 5 === 0){
      const before = enemySteps;
      enemySteps = Math.max(0, enemySteps - 2);
      if(before !== enemySteps){ appendLog('疲劳的躯体：西斯特 本回合额外 -2 步'); updateStepsUI(); }
    }
...
function sisterAiCountClusterAround(target){
  if(!target) return 0;
  return enemyLivingPlayers().filter(p=>p.hp>0 && Math.max(Math.abs(p.r-target.r), Math.abs(p.c-target.c))<=2).length;
}
function sisterAiBestDirectionalSkill(en, sk, baseScore){
  let best=null;
  for(const d of Object.keys(DIRS)){
    const cells = computeCellsForSkill(en, sk, d);
    const targets = sisterTargetsInCells(cells);
    if(!targets.length) continue;
    let colorScore=0, bleedScore=0, weakScore=0;
    for(const t of targets){
      colorScore += sisterTotalColorLayers(t,true);
      bleedScore += (t.status && t.status.bleed) ? Math.min(6, t.status.bleed) : 0;
      weakScore += Math.round((1 - (t.hp/Math.max(1,t.maxHp))) * 10);
    }
    const sc = baseScore + targets.length*14 + Math.min(18,colorScore*2) + Math.min(12,bleedScore) + weakScore;
    if(!best || sc>best.score) best={sk, dir:d, score:sc, targets};
  }
  return best;
}
function buildSisterAiCandidates(en, skillset){
  const candidates=[];
  const paint = en.status ? (en.status.paint||0) : 0;
  const players = enemyLivingPlayers();
  const anyPlayer = players[0];
  const nearest = players.length ? players.slice().sort((a,b)=>distanceForAI(en,a)-distanceForAI(en,b))[0] : null;
  const attackNames = new Set(['随性作画','发泄性作画']);
  const hasPaintAttack = paint>0 && skillset.some(sk=>sk && attackNames.has(sk.name));

  const activeSkillset = skillset.some(sk=>sk && sk.name==='签名') ? skillset.filter(sk=>sk && sk.name==='签名') : skillset;
  for(const sk of activeSkillset){
    if(!sk) continue;
    if(sk.meta && sk.meta.consumeAllSteps){ if(enemySteps<=0) continue; }
    else if(sk.cost>enemySteps) continue;
    if(sk.name==='发布') { candidates.push({sk, dir:en.facing, score:200}); continue; }

    if(sk.name==='啧，又空了'){
      // 只要这张牌已经出现，就应该马上使用；它本身只会在颜料<2且手里没有时生成。
      candidates.push({sk, dir:en.facing, score: 190});
      continue;
    }
    ...
    if(sk.name==='签名'){
      ...
      let best=null;
      for(const c of cells){ const tu=getUnitAt(c.r,c.c); if(tu && tu.side==='player' && (!best || tu.hp<best.targetUnit.hp)) best={sk,targetUnit:tu,score:120}; }
      if(best) candidates.push(best);
      continue;
    }

    if(sk.name==='随性作画'){
      const best=sisterAiBestDirectionalSkill(en, sk, 92);
      if(best) candidates.push(best);
      continue;
    }
    if(sk.name==='发泄性作画'){
      const best=sisterAiBestDirectionalSkill(en, sk, 98);
      if(best) candidates.push(best);
      continue;
    }

    if(sk.name==='剧本创作'){
      // 剧本创作不是无限冷却，而是每回合最多 2 次：
      // 有牌、有步数、有目标就应该会用；但不要压过“签名/发布”，也不要一回合全在传送。
      if((en._sisterScriptUsesThisTurn||0) >= 2) continue;
      const cells = sk.rangeFn(en, en.facing, null) || [];
      let best=null;
      const scriptCardsInHand = skillset.filter(s2=>s2 && s2.name==='剧本创作').length;
      for(const c of cells){
        const tu=getUnitAt(c.r,c.c);
        if(!tu || tu.side!=='player' || tu.hp<=0) continue;
        const cluster=sisterAiCountClusterAround(tu);
        const colorLayers=sisterTotalColorLayers(tu,true);
        const bleed=(tu.status && tu.status.bleed) ? tu.status.bleed : 0;
        let sc = 74 + cluster*12 + Math.min(10, bleed*2) + Math.min(10, colorLayers);
        if(scriptCardsInHand>=2) sc += 14;           // 手牌堆了很多剧本创作时，优先清掉
        if((en._sisterScriptUsesThisTurn||0)===1) sc -= 18; // 第二次可用，但降权
        if(hasPaintAttack && paint>0) sc -= 6;      // 仍然让颜色技能更常见，但不会完全压死剧本
        if(!best || sc>best.score) best={sk,targetUnit:tu,score:sc};
      }
      if(best) candidates.push(best);
      continue;
    }
    ...
    if(sk.name==='这里是我的画布'){
      // 画布是接近/重置站位工具：如果剧本创作很多但够不到人，就允许用它接近；贴脸可打人时不乱传。
      const immediateHit = skillset.some(s2=>attackNames.has(s2.name) && Object.keys(DIRS).some(d=>(sisterTargetsInCells(computeCellsForSkill(en,s2,d)).length>0)));
      const scriptCardsInHand = skillset.filter(s2=>s2 && s2.name==='剧本创作').length;
      const scriptHasTarget = skillset.some(s2=>s2 && s2.name==='剧本创作' && (s2.rangeFn(en,en.facing,null)||[]).some(c=>{ const tu=getUnitAt(c.r,c.c); return tu && tu.side==='player' && tu.hp>0; }));
      if(immediateHit || scriptHasTarget) continue;
      const far = nearest ? distanceForAI(en, nearest) : 99;
      const score = scriptCardsInHand>=2 ? 70 : (far>=4 ? 50 : 12);
      candidates.push({sk, dir:en.facing, score});
      continue;
    }
    ...
    for(const d of Object.keys(DIRS)){
      const cells=computeCellsForSkill(en,sk,d);
      const targets=sisterTargetsInCells(cells);
      if(targets.length) candidates.push({sk,dir:d,score:20+targets.length*8});
    }
  }
...
function buildSkillCandidates(en){
  if(en && en.id==='sister'){
    const sisterFactories = buildSisterSkillFactories(en) || [];
    const forcedFactory = sisterFactories.length === 1 && (sisterFactories[0].key === '发布' || sisterFactories[0].key === '签名') ? sisterFactories[0] : null;
    if(forcedFactory){
      en.skillPool = [forcedFactory.make()];
    }
  }
  const skillset = (en.skillPool && en.skillPool.length) ? en.skillPool : [];
  if(en && en.id==='sister') return buildSisterAiCandidates(en, skillset);
  const candidates=[];
...
async function execEnemySkillCandidate(en, cand){
  const aiCost = (cand.sk.meta && cand.sk.meta.consumeAllSteps) ? enemySteps : cand.sk.cost;
  enemySteps = Math.max(0, enemySteps - aiCost);
  updateStepsUI();

  const cells = cand.targetUnit
    ? [{r:cand.targetUnit.r, c:cand.targetUnit.c}]
    : computeCellsForSkill(en, cand.sk, cand.dir);
...
async function enemyTurn(){
  ...
  appendLog('敌方开始行动');
  const sisterUnit = units['sister'];
  if(sisterUnit) sisterUnit._sisterScriptCooldown = 0; // 旧版冷却废弃：现在改成每回合最多 2 次
...
function checkWin(){
  ...
  if(!enemiesAlive){ 
    stop西斯特BGM();
    appendLog('胜利！返回关卡选择');
    // Award coins for completing sister stage
    if (typeof localStorage !== 'undefined') {
      ...
      const completions = JSON.parse(localStorage.getItem(STORAGE_KEY_STAGE_COMPLETIONS) || '{"intro":0,"abandonedAnimals":0,"fatigue":0,"sevenSeas":0,"sisterBattle":0}');
      
      // Increment sister completions
      completions.sisterBattle = (completions.sisterBattle || 0) + 1;
      localStorage.setItem(STORAGE_KEY_STAGE_COMPLETIONS, JSON.stringify(completions));
      
      // Award 6 coins for sister
      const newCoins = currentCoins + 6;
      localStorage.setItem(STORAGE_KEY_COINS, newCoins.toString());
      
      appendLog('获得 6 币！（总计: ' + newCoins + ' 币）');
    }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  sisterBGMPhase1 = document.getElementById('sisterBGMPhase1');
  sisterBGMPhase2 = document.getElementById('sisterBGMPhase2');
  startBattleOverlay = document.getElementById('startBattleOverlay');
  ...
  appendLog('西斯特：图书馆画布，地图 29x20；六个画框按手绘图作为边框掩体。');
  appendLog('西斯特：HP 550 / SP 125 / 护甲15；SP降到 -45 会自损20真伤并失控。');
  appendLog('颜色状态会按文档结算：蓝/黑回合消耗，红绿紫受击反应，黄橙出伤反应。');
  ...
  const startBattle = ()=>{
    if(sisterBGMUnlocked) return;
    sisterBGMUnlocked = true;
    if(startBattleOverlay) startBattleOverlay.classList.add('hidden');
    playIntroCinematic();
  };
  // 不再显示“开始战斗”遮罩：进入关卡后自动开始。
  // 音乐如果被浏览器自动播放限制挡住，会在玩家第一次点击页面时自动补播。
  if(startBattleBtn){
    startBattleBtn.addEventListener('click', startBattle, {once:true});
  }
