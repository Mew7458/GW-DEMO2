...
// Prefer sessionStorage so two tabs on the same browser won't overwrite each other's role/room/host.
const farPvpRuntimeStorage = (() => {
  try { return window.sessionStorage; } catch (e) { return null; }
})();
function farPvpRuntimeGet(key){
  try {
    const v = farPvpRuntimeStorage ? farPvpRuntimeStorage.getItem(key) : null;
    if(v !== null && v !== undefined && v !== '') return v;
  } catch(e){}
  // Only fall back to localStorage if sessionStorage is not available at all.
  // (If sessionStorage exists but is empty, we intentionally do NOT read localStorage,
  // otherwise two tabs can overwrite each other's role and jump into the wrong side.)
  if(!farPvpRuntimeStorage){
    try { return localStorage.getItem(key) || ''; } catch(e){}
  }
  return '';
}

const localRole = farPvpRuntimeGet(LOCAL_ROLE_KEY) || 'player1';
const localSide = localRole === 'player2' ? 'enemy' : 'player';
function isLocalTurn(){
  return currentSide === localSide;
}
...
function sideLabel(side){ return side==='player' ? '玩家1' : '玩家2'; }
function normalizeCombatId(id){ return String(id || '').replace(/_p2$/,''); }
function addBonusStepsNextTurn(side, amount){
  if(!bonusStepsNextTurn) bonusStepsNextTurn = { player:0, enemy:0 };
  if(typeof bonusStepsNextTurn[side] !== 'number') bonusStepsNextTurn[side] = 0;
  bonusStepsNextTurn[side] += amount;
}
function consumeBonusSteps(side){
  if(!bonusStepsNextTurn) return 0;
  const b = bonusStepsNextTurn[side] || 0;
  bonusStepsNextTurn[side] = 0;
  return b;
}
...
const battleSync = {
  ...
  // Remote action playback (animate state diffs in order)
  animQueue: [],
  animPlaying: false,
...
// In Duo Mode, this battle runs inside an iframe (Menu/index.html).
// Redirecting to index.html here would load the *main menu* inside the iframe.
// Instead, ask the parent page to close the battle and go back to stages.
function requestReturnToMenu(){
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'GW_FARPVP_BATTLE_FINISHED' }, '*');
      return;
    }
  } catch (e) {
    // ignore
  }
  // Fallback: standalone battle page
...
function isVoidCell(){ return false; }
const coverCells = new Set();
function addCoverCellTL(x, y){
  const r = y;
  const c = x;
...
function addCoverRectTL(x1, y1, x2, y2){
...
units['karma'] = createUnit('karma','Karma','player',70, 10, 4, 200,50, 0.5,20, ['violentAddiction','toughBody','pride'], {stunThreshold:2});
// 玩家2
units['adora_p2'] = createUnit('adora_p2','Adora','enemy',70, 7, 15, 100,100, 0.5,0, ['backstab','calmAnalysis','proximityHeal','fearBuff'], {stunThreshold:2});
units['dario_p2'] = createUnit('dario_p2','Dario','enemy',70, 4, 14, 150,100, 0.75,0, ['quickAdjust','counter','moraleBoost'], {stunThreshold:2});
units['karma_p2'] = createUnit('karma_p2','Karma','enemy',70, 10, 14, 200,50, 0.5,20, ['violentAddiction','toughBody','pride'], {stunThreshold:2});
...
function buildAttackSwingFx({anchor, angle, config}){
  ...
  const spread = ((config.spread) !== undefined && (config.spread) !== null) ? (config.spread) : (16);
  const delayBase = ((config.delayBase) !== undefined && (config.delayBase) !== null) ? (config.delayBase) : (0);
  const delayStep = ((config.delayStep) !== undefined && (config.delayStep) !== null) ? (config.delayStep) : (40);
  arcs.forEach((el, i)=>{
...
function deriveAttackFxConfig(config){
  ...
  switch(config.type){
    case 'slash':{
      ...
      const spread = ((config.attackSpread) !== undefined && (config.attackSpread) !== null) ? (config.attackSpread) : ((variant)=== 'wide' ? 22 : 16);
      return {type:'swing', swings, spread, delayStep: swings>1 ? 34 : 0, variant};
    }
    case 'claw':{
      const swings = Math.max(1, Math.min(4, config.scratches || 3));
      const spread = ((config.attackSpread) !== undefined && (config.attackSpread) !== null) ? (config.attackSpread) : (14);
      const variant = config.variant === 'mecha' ? 'mecha' : 'claw';
      return {type:'swing', swings, spread, delayStep: ((config.delayStep) !== undefined && (config.delayStep) !== null) ? (config.delayStep) : (26, variant)};
    }
    case 'beam':{
...
function buildClawSkillFx({anchor, angle, config}){
  ...
  const scratchSpacing = ((config.spacing) !== undefined && (config.spacing) !== null) ? (config.spacing) : (16);
  const scratchDelay = ((config.delayStep) !== undefined && (config.delayStep) !== null) ? (config.delayStep) : (30);
  const scratchBaseDelay = ((config.delayBase) !== undefined && (config.delayBase) !== null) ? (config.delayBase) : (0);
  let scratchHtml='';
  ...
  const shardSpread = ((config.shardSpread) !== undefined && (config.shardSpread) !== null) ? (config.shardSpread) : (22);
  const shardArc = ((config.shardArc) !== undefined && (config.shardArc) !== null) ? (config.shardArc) : (18);
  const shardStart = ((config.shardStartAngle) !== undefined && (config.shardStartAngle) !== null) ? (config.shardStartAngle) : (-26);
...
function startBattleBGM(){
  ...
  if(bossBGM.readyState >= 2){
    bossBGM.play().catch(e => console.log('BGM autoplay blocked:', e));
  } else {
...
function initBattleSync(){
  const roomId = farPvpRuntimeGet(LOCAL_ROOM_KEY);
...
function restoreSkillPoolFromRemote(u, serializedPool){
  ...
  u.skillPool = rebuilt;
  // 额外技能（黑瞬「释放」、爆等）不占普通13张上限；只裁剪非额外卡。
  try { trimSkillPool(u); } catch(e) {}
}
...
function applyBattleStateInstant(state){
  ...
  Object.entries(state.units).forEach(([id, data]) => {
    ...
    u.r = data.r;
    u.c = data.c;
    u.hp = Math.max(0, Math.min(u.maxHp, Number(data.hp) || 0));
    u.sp = Math.max(0, Math.min(u.maxSp, Number(data.sp) || 0));
...
function captureDiffSnapshot(){
  const snap = { currentSide, playerSteps, enemySteps, roundsPassed, units: {} };
  try{
    for(const id in units){
      const u = units[id];
      if(!u) continue;
      snap.units[id] = { r: u.r, c: u.c, hp: u.hp, sp: u.sp, facing: u.facing };
    }
  }catch(e){}
  return snap;
}

function animateRemoteDiff(prevSnap, nextState, done){
  // nextState is the full captured state payload from the other player.
  const nextUnits = (nextState && nextState.units) ? nextState.units : {};
  const moves = [];
  const changes = [];
  try{
    for(const id in nextUnits){
      const prev = prevSnap.units[id];
      const next = nextUnits[id];
      if(!prev || !next) continue;
      if(prev.r !== next.r || prev.c !== next.c){
        moves.push({ id, from: prev, to: next });
      }
      if(prev.hp !== next.hp || prev.sp !== next.sp){
        changes.push({ id, prev, next });
      }
    }
  }catch(e){}

  const backlog = battleSync.animQueue.length;
  const fast = backlog >= 2;
  const trailDur = fast ? 240 : 420;
  const moveApplyDelay = fast ? 70 : 120;
  const impactDelay = fast ? 110 : 170;
  const finishDelay = fast ? 300 : 560;

  // Step counters should also sync during replay (so the waiting player can see step usage).
  try {
    const prevPS = typeof prevSnap.playerSteps === 'number' ? prevSnap.playerSteps : playerSteps;
    const prevES = typeof prevSnap.enemySteps === 'number' ? prevSnap.enemySteps : enemySteps;
    const nextPS = (nextState && typeof nextState.playerSteps === 'number') ? nextState.playerSteps : prevPS;
    const nextES = (nextState && typeof nextState.enemySteps === 'number') ? nextState.enemySteps : prevES;
    if (prevPS !== nextPS || prevES !== nextES) {
      const stepApplyAt = moves.length ? moveApplyDelay : (changes.length ? impactDelay : 0);
      setTimeout(() => {
        playerSteps = nextPS;
        enemySteps = nextES;
        updateStepsUI();
  ...
  // Lock interactions while replaying (even if the turn already switched locally).
  ...
  if(moves.length){
    for(const m of moves){
      ...
      if(!u) continue;
      try{
        showTrailWithDuration(m.from.r, m.from.c, m.to.r, m.to.c, trailDur, { thickness: 8 });
        cameraFocusOnCell(m.to.r, m.to.c);
      }catch(e){}
      setTimeout(()=>{
        const uu = units[m.id];
        if(!uu) return;
        uu.r = m.to.r; uu.c = m.to.c;
        if(m.to.facing) uu.facing = m.to.facing;
        try{ renderAll(); pulseCell(uu.r, uu.c); }catch(e){}
      }, moveApplyDelay);
    }
  }
  ...
  setTimeout(()=>{
    for(const ch of changes){
      const u = units[ch.id];
      if(!u) continue;
      const hpDelta = (ch.next.hp|0) - (ch.prev.hp|0);
      const spDelta = (ch.next.sp|0) - (ch.prev.sp|0);

      try{
        if(hpDelta < 0 || spDelta < 0){
          showDamageFloat(u, hpDelta < 0 ? Math.abs(hpDelta) : 0, spDelta < 0 ? Math.abs(spDelta) : 0);
        }
        if(hpDelta > 0 || spDelta > 0){
          showGainFloat(u, hpDelta > 0 ? hpDelta : 0, spDelta > 0 ? spDelta : 0);
        }
        pulseCell(u.r, u.c);
      }catch(e){}
    }
  ...
  setTimeout(()=>{ done && done(); }, finishDelay);
}

function queueRemoteBattleState(state){
  battleSync.animQueue.push(state);
  if(battleSync.animPlaying) return;
  battleSync.animPlaying = true;

  const pump = ()=>{
    const next = battleSync.animQueue.shift();
    if(!next){
      battleSync.animPlaying = false;
      // Unlock if it's your turn; otherwise keep locked only by normal turn rules.
      interactionLocked = false;
      renderAll();
      return;
    }

    if(!battleSync.hasInitialSync){
      applyBattleStateInstant(next);
      battleSync.hasInitialSync = true;
      interactionLocked = false;
      setTimeout(pump, 0);
      return;
    }

    const prev = captureDiffSnapshot();
    animateRemoteDiff(prev, next, ()=>{
      applyBattleStateInstant(next);
      interactionLocked = false;
      setTimeout(pump, 0);
    });
...
// Wrapper: apply battle state with ordered playback for remote updates.
function applyBattleState(state){
  // For local-only / immediate application, callers can use applyBattleStateInstant directly.
  if(!state){ return; }
  queueRemoteBattleState(state);
}
...
function syncBattleStateFromRoom(room){
  ...
  // Global monotonic version: each client advances their local counter based on the latest seen version.
  if(version <= battleSync.lastAppliedStateVersion) return;
  battleSync.lastAppliedStateVersion = version;
  battleSync.stateVersion = Math.max(battleSync.stateVersion || 0, version);
...
function showRollOverlay(){
  ...
  if(battleAreaEl) battleAreaEl.classList.add('is-blurred');
}

function hideRollOverlay(){
  if(!rollOverlayEl) return;
  rollOverlayEl.classList.add('hidden');
  if(battleAreaEl) battleAreaEl.classList.remove('is-blurred');
}
...
function handleRoll(playerKey){
  if(playerKey !== localRole){
    appendLog('只能摇自己的数字');
    return;
  }
  if(rollState.rolling[playerKey]) return;
  const box = document.querySelector(`.rollBox[data-player="${playerKey}"]`);
  const button = document.querySelector(`.rollBtn[data-player="${playerKey}"]`);
  if(!box || !button) return;

  rollState.rolling[playerKey] = true;
...
function handleSpCrashIfNeeded(u){
  ...
  if(u.sp <= 0 && !u._spBroken){
    ...
    u.spPendingRestore = Math.max(((u.spPendingRestore) !== undefined && (u.spPendingRestore) !== null) ? (u.spPendingRestore) : (0, restored));
...
function backstabMultiplier(attacker,target){
  ...
  if(normalizeCombatId(attacker.id) === 'adora' && attacker.sp < 10) return 1.5;
  return 1.0;
}
function hasDeepBreathPassive(attacker){
  if(!attacker || normalizeCombatId(attacker.id)!=='karma') return false;
  const pool = attacker.skillPool || [];
  return pool.some(s=>s && s.name === '深呼吸');
}
function hasBloomInSidePool(side){
  // 在同一阵营的技能池里（任意角色）一次性只能有 1 个「绽放（红色）」。
  const ids = side === 'enemy'
    ? ['adora_p2', 'dario_p2', 'karma_p2']
    : ['adora', 'dario', 'karma'];
  for (const id of ids) {
    const u = units[id];
    if (!u || u.hp <= 0) continue;
...
// 兼容旧逻辑：部分技能/日志里仍会调用 hasBloomInAnyPlayerPool()
// 在 PvP 中它等价于「玩家1阵营是否拥有绽放」。
function hasBloomInAnyPlayerPool(){
  return hasBloomInSidePool('player');
}
function calcOutgoingDamage(attacker, baseDmg, target, skillName){
  ...
  if(normalizeCombatId(attacker.id)==='karma' && skillName==='沙包大的拳头' && (attacker.consecAttacks||0)>=1){ dmg = Math.round(dmg*1.5); }
  if(normalizeCombatId(attacker.id)==='adora' && skillName==='短匕轻挥' && target){ dmg = Math.round(dmg * backstabMultiplier(attacker,target)); }
...
function damageUnit(id, hpDmg, spDmg, reason, sourceId=null, opts={}){
  ...
  if(!trueDamage){
    ...
    const baseId = String(u.id||'').replace('_p2','');
    if(equipped[baseId] === "vest"){
...
if(u._spCrashVuln && (hpDmg>0 || spDmg>0)){
  ...
  if(sourceId){
    const src = units[sourceId];
    // PvP：两边都是玩家，所有“攻击触发的被动/饰品效果”应当对称生效
    if(src && (finalHp>0 || finalSp>0)){
      ...
      // Bloom (Red) passive: 当「施放者所属阵营」的技能池里存在绽放时，队友攻击敌人叠加血色花蕾
      if(u.side !== src.side && hasBloomInSidePool(src.side)){
...
function darioSweetAfterBitter(u){
  addBonusStepsNextTurn(u.side, 4);
  appendLog(`${u.name} 使用 先苦后甜：下个${sideLabel(u.side)}回合 +4 步`);
...
// Far PvP skill selection injection (for Safari file:// where localStorage may not be shared across iframes)
let __gwInjectedSkillSelection = null;

// Also support passing selections via URL param (most reliable under file:// Safari)
// Example: farpvp-battle.html?mode=farpvp&farpvpsel=<base64(json)>
(function hydrateInjectedSelectionFromUrl(){
  try {
    const qs = (window.location && window.location.search) ? window.location.search : '';
    if(!qs) return;
    const params = new URLSearchParams(qs);
    const raw = params.get('farpvpsel');
    if(!raw) return;
    const json = decodeURIComponent(escape(atob(decodeURIComponent(raw))));
    const obj = JSON.parse(json);
    if(obj && typeof obj === 'object'){
      __gwInjectedSkillSelection = { mode:'farpvp', data: obj };
    }
  } catch(e){
    // ignore
  }
...
window.addEventListener('message', (event) => {
  ...
  if (!data || data.type !== 'GW_FARPVP_SELECTED_SKILLS') return;
  if (data.selections && typeof data.selections === 'object') {
    __gwInjectedSkillSelection = { mode: 'farpvp', data: data.selections };
  }
});
function loadSelectedSkillsForBattle() {
  try {
    if (__gwInjectedSkillSelection) return __gwInjectedSkillSelection;

    const farSaved = localStorage.getItem('gwdemo_farpvp_selected_skills');
    if (farSaved) {
      return { mode: 'farpvp', data: JSON.parse(farSaved) };
    }
...
// Helper function to check if skill selection should be applied.
// - Single-player: keep the original "level 50+" gate.
// - FarPVP/Duo: ALWAYS apply when selections exist (PvP skills must follow the skill-selection screen).
function shouldApplySkillSelection(u) {
  const sel = loadSelectedSkillsForBattle();
  if (sel && (sel.mode === 'farpvp' || sel.mode === 'duo')) return true;
  return u && u.level >= 50;
}
...
function getSelectedSkillKeysForUnit(u) {
  ...
  if (!selectedSkills || !selectedSkills.data) return null;
  if (!shouldApplySkillSelection(u)) return null;

  const baseId = u.id.replace('_p2', '');

  let charSelection = null;
  if (selectedSkills.mode === 'farpvp' || selectedSkills.mode === 'duo') {
    const playerKey = u.id.endsWith('_p2') ? 'player2' : 'player1';
    charSelection = selectedSkills.data?.[playerKey]?.[baseId] || null;
  } else {
    charSelection = selectedSkills.data?.[baseId] || null;
  }
  // If no selection record exists, don't filter (fallback to default skill pool).
...
function summarizeNegatives(u){
  ...
  // Display equipped accessory
  {
    const equipped = loadEquippedAccessories();
    const baseId = String(u.id||'').replace('_p2','');
    const accessoryId = equipped[baseId];
    if(accessoryId){
...
async function handleSkillConfirmCell(u, sk, aimCell){
  ...
  }catch(e){ setInteractionLocked(false); console.error('技能执行错误',e); appendLog(`[错误] 技能执行失败：${sk.name} - ${e.message}`); }
  ...
  if(normalizeCombatId(u.id)==='karma' && sk.name!=='沙包大的拳头'){
...
function onCellClick(r,c){
  ...
  pulseCell(r,c);
  if(normalizeCombatId(sel.id)==='karma' && sel.consecAttacks>0){ appendLog(`${sel.name} 的连击被打断（移动）`); sel.consecAttacks=0; }
  unitActed(sel);
...
function showSelected(u){
  ...
  if(skillPool){
    if(!isLocalTurn()){
      skillPool.innerHTML = `<div class="partyRow small">等待对方操作中...</div>`;
    } else if(u.side!==currentSide){
      ...
      for(const sk of pool){
        ...
        if(!stepsOk || !isLocalTurn()) card.classList.add('disabled');
        ...
        discardBtn.addEventListener('click',(e)=>{ e.stopPropagation(); if(interactionLocked || !isLocalTurn()) return; discardSkill(u, sk); });
        ...
        card.addEventListener('contextmenu',(e)=>{ e.preventDefault(); if(interactionLocked || !isLocalTurn()) return; discardSkill(u,sk); });
        card.addEventListener('click', ()=>{
          if(interactionLocked || !isLocalTurn()) return;
...
function processUnitsTurnEnd(side){
  for(const id in units){
    ...
    if(normalizeCombatId(u.id)==='adora' && u.passives.includes('calmAnalysis')){
    ...
    if(normalizeCombatId(u.id)==='karma' && u.consecAttacks>0){ appendLog('Karma 连击在回合结束时重置'); u.consecAttacks=0; }
  }
  for(const id in units){
    const u=units[id];
    if(u.side!==side) continue;
...
function finishEnemyTurn(){
  ...
  setTimeout(()=>{
    ...
    const bonus = consumeBonusSteps('player');
    if(bonus>0){
      playerSteps += bonus;
      appendLog(`先苦后甜：${sideLabel('player')}额外 +${bonus} 步`);
    }
...
function renderAll(){
  // 远程收到对方快照时不要在应用快照过程中再次本地改状态；
  // 墨片拾取由行动端 unitActed()/移动后先结算，再同步给另一端。
  if(!(typeof battleSync !== 'undefined' && battleSync.applyingRemote)) consumeInkShardsUnderUnits();
  buildGrid();
  placeUnits();
  renderStatus();
  updateStepsUI();
...
function checkEndOfTurn(){
  if(currentSide==='player' && playerSteps<=0){
    ...
    const bonusE = consumeBonusSteps('enemy');
    if(bonusE>0){
      enemySteps += bonusE;
      appendLog(`先苦后甜：${sideLabel('enemy')}额外 +${bonusE} 步`);
    }
    applyLevelSuppression();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  rollOverlayEl = document.getElementById('rollOverlay');

  // 进入战斗后：50% 播放 DuoBattle.mp3，50% 播放 DuoBattle2.mp3
  if (bossBGM) {
    const bgmPick = Math.random() < 0.5 ? 'DuoBattle.mp3' : 'DuoBattle2.mp3';
    try {
      bossBGM.src = bgmPick;
      bossBGM.load();
    } catch (e) {
  ...
  appendLog('双人对战开始：地图 17x13；掩体为不可进入。');
  appendLog('叠层眩晕：双方单位需要 2 层叠层触发 1 层眩晕 Debuff。');

  const endTurnBtn=document.getElementById('endTurnBtn');
  if(endTurnBtn) endTurnBtn.addEventListener('click', ()=>{ if(interactionLocked || !isLocalTurn()) return; endTurn(); });
  document.querySelectorAll('.rollBtn').forEach((btn) => {
    btn.addEventListener('click', () => handleRoll(btn.dataset.player));
  });
  ...
  godsWillBtn.onclick = (e)=>{
    ...
    if(!godsWillUnlocked){
      ...
      const normalized = (((answer) !== undefined && (answer) !== null) ? (answer) : ('').trim());
