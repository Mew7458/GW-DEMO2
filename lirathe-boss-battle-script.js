// 2D 回合制 RPG Demo - 旧情未了 - Old Love Unfinished
// 变更摘要：
// - 重制敌方阵容：移除七海小队，改为 Lirathe 的单体 Boss 战与 10x20 新地图。
// - Lirathe：实现“老干部”“变态躯体”“疲劳的躯体”“糟糕的初始设计”等被动与六种招式。
// - 新状态“怨念”：在回合开始蚕食目标 5% SP，并可被痛苦咆哮清算。
// - SP 系统：支持单位自定义 SP 下限以及禁用通用崩溃，新增疲劳崩溃逻辑。
// - AI 调整：Lirathe 达到移动上限却仍无法攻击时触发全场 -10 SP 惩罚；保留 BFS+兜底消步机制。
...
const LIRATHE_FATIGUE_STUN_DURATION = 3;
...
let webTrapRevealed = new Set();  // traps stay invisible until triggered
let specialSpawnsDisabled = false;
...
function randomEmptyCell({avoidUnits=true, avoidSpecial=true}={}){
  const cells=[];
  for(let r=1;r<=ROWS;r++){
    for(let c=1;c<=COLS;c++){
      if(!clampCell(r,c)) continue;
      const k=cellKey(r,c);
      if(avoidUnits && getUnitAt(r,c)) continue;
      if(avoidSpecial && (weakSpots.has(k) || recoveryTiles.has(k) || webTraps.has(k))) continue;
      cells.push({r,c});
    }
  }
...
function coveredKeysForUnit(u){
  if(!u) return [];
  const cells = (u.size===2) ? [{r:u.r,c:u.c},{r:u.r+1,c:u.c},{r:u.r,c:u.c+1},{r:u.r+1,c:u.c+1}] : [{r:u.r,c:u.c}];
  return cells.filter(p=>p.r>=1 && p.r<=ROWS && p.c>=1 && p.c<=COLS).map(p=>cellKey(p.r,p.c));
}
...
// 疲惫的极限 Boss
// 旧情未了 Boss - Phase 1
units['lirathe'] = createUnit('lirathe','Lirathe','enemy',50, 5, 5, 700, 80, 0.94, 20, ['dancerDream','stingingHeart','swiftAgile','reborn','trulyUnwilling'], {
  size:1,
  stunThreshold:4,
  spFloor:0,
...
function range_footprint_square(u, padding){
  const arr=[];
  const height = u.size===2 ? 2 : 1;
  const width = u.size===2 ? 2 : 1;
  for(let r=u.r-padding; r<=u.r+height-1+padding; r++){
    for(let c=u.c-padding; c<=u.c+width-1+padding; c++){
      if(clampCell(r,c)) arr.push({r,c,dir:u.facing});
    }
  }
  return arr;
}
...
const SKILL_FX_CONFIG = {
  ...
  'lirathe:血肉之刃':       {type:'slash', primary:'#ff6f6f', secondary:'rgba(255,180,180,0.7)', spark:'#ffd6d6', slashes:2},
  'lirathe:怨念之爪':       {type:'claw', primary:'#b168ff', secondary:'#e7d4ff', scratches:3},
  'lirathe:蛮横横扫':       {type:'slash', primary:'#ff964f', secondary:'rgba(255,205,165,0.7)', spark:'#ffe7c8', slashes:3, attack:{type:'swing', swings:3, spread:28, delayStep:34, variant:'wide', faceTarget:false}},
  'lirathe:能者多劳':       {type:'slash', primary:'#ff4f88', secondary:'rgba(255,168,205,0.7)', spark:'#ffd1e4', slashes:4, attack:{type:'swing', swings:4, spread:26, delayStep:30, variant:'wide', faceTarget:false}},
  'lirathe:痛苦咆哮':       {type:'burst', primary:'#af7bff', secondary:'#e4d4ff', glyph:'怨'},
  'lirathe:过多疲劳患者最终的挣扎': {type:'burst', primary:'#ffbf5f', secondary:'#ffe6b9', glyph:'终'},
  'lirathe:疲劳崩溃':       {type:'impact', primary:'#bfbfbf', secondary:'#f5f5f5'},
};
...
async function playIntroCinematic(){
  ...
  await showIntroLine('Lirathe：......');
  await showIntroLine('Lirathe：你不应该在这里......');
  hideIntroDialog();
  cameraReset();
  await sleep(520);
  showRoundBanner('回合一', 1800);
  
  // Play Lirathe BGM after "Round One" banner appears
  if (liratheBGM && !liratheBGMPlayed) {
    try {
      liratheBGMPlayed = true;
      liratheBGM.volume = 0.7;
      await liratheBGM.play();
      appendLog('🎵 Lirathe 主题曲开始播放');
    } catch (err) {
...
// Helper function to stop Lirathe BGM
function stopLiratheBGM() {
  if (liratheBGM && !liratheBGM.paused) {
    liratheBGM.pause();
    liratheBGM.currentTime = 0;
    appendLog('🎵 Lirathe 主题曲停止播放');
  }
}
...
function checkLiratheFatigue(u){
  if(!u || u.id!=='lirathe' || u.hp<=0) return;
  if(u._fatigueCrashLock) return;
  if(u.sp <= -100){
    u._fatigueCrashLock = true;
    appendLog(`${u.name} 的“疲劳的躯体”崩溃：SP 跌至 -100`);
    u._highGroundTurns = 0;
    damageUnit(u.id, 20, 0, `${u.name} 疲劳崩溃`, u.id, {trueDamage:true, ignoreToughBody:true, ignoreHighGround:true, skillFx:'lirathe:疲劳崩溃'});
    const newStunValue = (u.status.stunned || 0) + 1;
    updateStatusStacks(u, 'stunned', newStunValue, {label:'眩晕', type:'debuff'});
    u._spCrashVuln = true;
    appendLog(`${u.name} 因疲劳崩溃，眩晕 +1 层（当前 ${newStunValue}），SP恢复到 -10，并进入1.5倍易伤`);
    if(u.side==='enemy'){
...
function applyLiratheDesignPenalty(){
...
function damageUnit(id, hpDmg, spDmg, reason, sourceId=null, opts={}){
  ...
  if(source){
    ...
    // Lirathe 被动：丧失理智 / 刺痛的心 / 真的好不甘心（攻击方加成）
    if(source.id === 'lirathe' && !opts.ignoreLiratheRage){
      if(lirathePhase === 1 && source.passives && source.passives.includes('trulyUnwilling') && source.hp <= source.maxHp * 0.5){
        hpDmg = Math.round(hpDmg * 1.45);
        spDmg = Math.round(spDmg * 1.45);
        appendLog(`${source.name} 的“真的好不甘心”触发：伤害+45%`);
      }
      const painStacks = Math.max(0, source._karmaPainStacks || 0);
      if(painStacks > 0){
        const painMult = 1 + painStacks * 0.0025;
        hpDmg = Math.round(hpDmg * painMult);
        spDmg = Math.round(spDmg * painMult);
      }
      if(lirathePhase === 2 && source.passives && source.passives.includes('lostReason') && Math.random() < 0.25){
        hpDmg = Math.round(hpDmg * 1.25);
        spDmg = Math.round(spDmg * 1.25);
        source.hp = Math.max(1, source.hp - 25);
        source.sp = Math.max(source.spFloor ?? 0, source.sp - 10);
        appendLog(`${source.name} 的“丧失理智”触发：伤害+25%，自身 -25HP -10SP`);
        showDamageFloat(source, 25, 10);
      }
    }
  ...
  // Lirathe 高处：普通攻击完全打不到，必须先通过软肋/SP等机制让她掉下来。
  if(u.id === 'lirathe' && u._highGroundTurns > 0 && source && source !== u && !opts.ignoreHighGround){
    appendLog(`${u.name} 正在墙面高处，免疫了这次攻击！`);
    showStatusFloat(u,'高处免疫',{type:'buff', offsetY:-60});
    pulseCell(u.r,u.c);
    renderAll();
    return;
  }
  ...
  if(!trueDamage && u.id==='lirathe'){
    ...
    if(lirathePhase === 1 && u.passives.includes('dancerDream') && Math.random() < 0.30){
      ...
      const options = range_move_radius(u, 1).filter(p=>!getUnitAt(p.r,p.c));
      if(options.length){
        options.sort((a,b)=> mdist(a, source||{r:oldR,c:oldC}) - mdist(b, source||{r:oldR,c:oldC}));
        const dest = options[options.length-1];
        u.r = dest.r; u.c = dest.c;
        pulseCell(dest.r,dest.c);
        appendLog(`${u.name} 闪到 (${dest.r},${dest.c})`);
      } else {
        pulseCell(u.r,u.c);
      }
      renderAll();
    ...
    // Phase 2: 退去凡躯 - 常态减伤70%；被软肋打下来后降为35%减伤；20%概率回血5HP
    if(lirathePhase === 2){
      const reduction = (u._weakSpotVulnTurns && u._weakSpotVulnTurns > 0) ? 0.35 : 0.70;
      hpDmg = Math.round(hpDmg * (1 - reduction));
      spDmg = Math.round(spDmg * (1 - reduction));
      appendLog(`${u.name} 的"退去凡躯"减伤${Math.round(reduction*100)}%`);
      if(Math.random() < 0.20){
        const healAmount = 5;
  ...
  if(!trueDamage && u._weakSpotVulnTurns > 0 && (finalHp>0 || finalSp>0)){
    finalHp = Math.round(finalHp * 1.5);
    finalSp = Math.round(finalSp * 1.5);
    appendLog(`${u.name} 踩中软肋后暴露：承受 1.5 倍伤害！`);
  }
  if((opts.lockTargetAtOne || u._lockedAtOne) && finalHp >= u.hp){
    finalHp = Math.max(0, u.hp - 1);
  }
  ...
  u.hp = Math.max(u._lockedAtOne ? 1 : 0, u.hp - finalHp);
  const floor = (typeof u.spFloor === 'number') ? u.spFloor : 0;
  ...
  if(source && source.id === 'lirathe' && source.passives && source.passives.includes('reborn') && !opts._rebornProc && u.hp>0 && (finalHp>0 || finalSp>0) && Math.random() < 0.05){
    appendLog(`${source.name} 的“重获新生”触发：追击一次该攻击！`);
    damageUnit(u.id, Math.max(0, hpDmg), Math.max(0, spDmg), `${source.name} 重获新生追击 ${u.name}`, source.id, Object.assign({}, opts, {_rebornProc:true}));
  }

  // Lirathe 被动：刺痛的心（被 Karma 命中后，小概率叠加后续攻击怒意）
  if(source && source.id === 'karma' && u.id === 'lirathe' && lirathePhase === 1 && (finalHp>0 || finalSp>0) && Math.random() < 0.25){
    u._karmaPainStacks = (u._karmaPainStacks || 0) + 1;
    appendLog(`${u.name} 的“刺痛的心”触发：怒意层数 ${u._karmaPainStacks}（后续伤害微增）`);
  }

  // 意识花苞死亡：留下软肋格
  if(died && u.id && u.id.startsWith('consciousness_bud_')){
    weakSpots.add(cellKey(u.r,u.c));
    appendLog(`${u.name} 被击破，脚下出现“软肋”格！`);
  }
  if(finalHp>0 && u.id && u.id.startsWith('consciousness_bud_')){
  ...
  if(died && u.id === "lirathe" && lirathePhase === 1 && !liratheTransformed){
    ...
    appendLog(`${u.name} 变身为 2x2 Boss（占4格）！HP: 1500, SP: 0 (floor: -100)`);
  ...
  if(u.id === "lirathe" && lirathePhase === 2 && !finalSequenceTriggered && u.hp > 0 && u.hp <= 400){
    ...
    // Heal Lirathe to full and clear negative effects
    u.hp = u.maxHp;
    if(u.status){
      for(const k of ['stunned','paralyzed','bleed','bloodyBud','resentStacks','mockeryStacks','blastStacks']){
        if(u.status[k]) updateStatusStacks(u, k, 0, {label:k, type:'debuff'});
      }
    }
    u._staggerStacks = 0;
    u._spBroken = false;
    u._spCrashVuln = false;
    u._weakSpotVulnTurns = 0;
    u._vulnerabilityStacks = 0;
    u._highGroundTurns = 0;
    appendLog(`${u.name} HP 恢复至满，并清除所有负面效果！`);
    
    // Karma 进入剧情锁血：触发后无论腐蚀、流血、普通攻击都至少保留 1HP。
    const lockedKarma = units['karma'];
    if(lockedKarma){
      lockedKarma._lockedAtOne = true;
      lockedKarma.hp = Math.max(1, lockedKarma.hp);
      appendLog('Karma 进入剧情锁血：HP 不会低于 1。');
    }
    ...
    specialSpawnsDisabled = true;
    cleanupSpecialTilesAndBuds();
    appendLog("终局序列已启动...");
    renderAll();
    setTimeout(()=> runLiratheFinalSequence(), 120);
  }
  // 反伤姿态：反弹部分HP伤害
  ...
  if(sourceId){
    const src = units[sourceId];
    if(src && src.id==='lirathe' && src!==u && src.hp>0 && (finalHp>0 || finalSp>0)){
...
function markLiratheSkillUsed(u){
  if(!u) return;
  // Mark that Lirathe has used a skill this turn (for movement restriction)
  if(u.id === 'lirathe'){
...
function liratheCollectTargets(cells){
  const set=new Set();
  const arr=[];
  for(const c of cells){
    const tu=getUnitAt(c.r,c.c);
...
function addBladeLightStack(source, target, layers=1){
  if(!source || !target || target.hp<=0) return;
  const before = bladeLightStacks.get(target.id) || 0;
  const after = before + layers;
  bladeLightStacks.set(target.id, after);
  appendLog(`${target.name} 刀光层数: ${after}/5`);
  if(after >= 5){
    const stacks = after;
    const hp = stacks * 5;
    const sp = stacks * 5;
    appendLog(`${target.name} 刀光到达 ${stacks} 层，自动爆炸！`);
    damageUnit(target.id, hp, sp, `刀光爆炸（${stacks}层）`, source.id, {trueDamage:true, ignoreCover:true});
    const hp0 = source.hp, sp0 = source.sp;
    source.hp = Math.min(source.maxHp, source.hp + hp);
    source.sp = Math.min(source.maxSp, source.sp + sp);
    syncSpBroken(source);
    showGainFloat(source, source.hp-hp0, source.sp-sp0);
    appendLog(`${source.name} 吸收刀光：恢复 ${source.hp-hp0}HP / ${source.sp-sp0}SP`);
...
function lirathe_TryToEscape(u, payload){
  const dest = payload && payload.moveTo ? payload.moveTo : payload;
  if(!dest || dest.r===undefined){ appendLog('又想逃？：无效目标'); unitActed(u); return; }
  
  const nr = dest.r, nc = dest.c;
...
async function lirathe_SwordDance(u, dir){
  ...
  const hitStages = [];
  const markHit = (stage)=>{ hitStages[stage] = true; };
  ...
  for(const cell of area1){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 20, 0, `${u.name} 剑舞·前斩`, u.id);
      target._vulnerabilityStacks = (target._vulnerabilityStacks || 0) + 1;
      appendLog(`${target.name} 获得1层脆弱！`);
      u.dmgDone += 20;
      markHit(0);
    }
  }

  const reverse = rotateDirCounterClockwise(rotateDirCounterClockwise(dir));
  const area2 = forwardRectCentered(u, reverse, 3, 2);
  await stageMark(area2);
  for(const cell of area2){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 20, 0, `${u.name} 剑舞·反斩`, u.id);
      u.dmgDone += 20;
      markHit(1);
    }
  }
  ...
  for(const cell of area3){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 10, 0, `${u.name} 剑舞·3x3`, u.id);
      u.dmgDone += 10;
      markHit(2);
    }
  }
  ...
  await stageMark(area5);
  for(const cell of area5){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 15, 0, `${u.name} 剑舞·5x5`, u.id);
      addBladeLightStack(u, target, 1);
      u.dmgDone += 15;
      markHit(3);
    }
  ...
  const area7 = range_square_n(u, 3);
  await stageMark(area7);
  for(const cell of area7){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 15, 0, `${u.name} 剑舞·7x7`, u.id);
      addBladeLightStack(u, target, 1);
      u.dmgDone += 15;
      markHit(4);
    }
  ...
  if([0,1,2,3,4].every(i=>hitStages[i])){
    const next = addStatusStacks(u,'mockeryStacks',1,{label:'戏谑', type:'buff'});
    appendLog(`${u.name} 的剑舞每段都命中：获得戏谑Buff（${next}）`);
  }
  unitActed(u);
}
...
async function lirathe_SplashBlade(u, dir){
  ...
  if(firstHit){
    ...
    addBladeLightStack(u, firstHit, 1);
    u.dmgDone += 5;
  }
...
async function lirathe_WhereAreYou(u){
  const area = range_footprint_square(u, 2);  // 2x2本体向外扩2格 = 6x6
  await telegraphThenImpact(area);
...
// —— Lirathe 关卡未完成机制：花苞 / 软肋 / 恢复格 / 蛛网 / 终局 ——
function spawnConsciousnessBud(){
  if(specialSpawnsDisabled || lirathePhase !== 2) return;
  const cell = randomEmptyCell();
  if(!cell) return;
  consciousnessBudCounter += 1;
  const id = `consciousness_bud_${consciousnessBudCounter}`;
  const bud = createUnit(id, `意识花苞${consciousnessBudCounter}`, 'enemy', 45, cell.r, cell.c, 150, 0, 0, 0, ['consciousnessBud'], {disableSpCrash:true, maxMovePerTurn:0, initialSp:0});
  bud._lastDamagedRound = roundsPassed;
  units[id] = bud;
  ensureStartHand(bud);
  appendLog(`意识花苞在 (${cell.r},${cell.c}) 生长。击破后会留下软肋格。`);
}
function spawnRecoveryTile(){
  if(specialSpawnsDisabled || lirathePhase !== 2) return;
  const cell = randomEmptyCell();
  if(!cell) return;
  recoveryTiles.add(cellKey(cell.r, cell.c));
  appendLog(`小恢复格子出现在 (${cell.r},${cell.c})。我方踩上可恢复 50HP/50SP。`);
}
function maybeSpawnWebTrapNear(u, chance=0.25){
  if(specialSpawnsDisabled || lirathePhase !== 2 || Math.random() >= chance) return;
  const options = range_square_n(u, 2).filter(p=> clampCell(p.r,p.c) && !getUnitAt(p.r,p.c));
  if(!options.length) return;
  const cell = options[Math.floor(Math.random()*options.length)];
  const key = cellKey(cell.r, cell.c);
  if(weakSpots.has(key) || recoveryTiles.has(key) || webTraps.has(key)) return;
  webTraps.add(key);
  appendLog(`困境触发：某处生成了看不见的蛛网陷阱。`);
}
function processSpecialTileEntry(u){
  if(!u || u.hp<=0) return;
  const keys = coveredKeysForUnit(u);

  // 小恢复格：只给玩家/友方使用
  if(u.side === 'player'){
    for(const key of keys){
      if(recoveryTiles.has(key)){
        recoveryTiles.delete(key);
        const beforeHp = u.hp, beforeSp = u.sp;
        u.hp = Math.min(u.maxHp, u.hp + 50);
        u.sp = Math.min(u.maxSp, u.sp + 50);
        syncSpBroken(u);
        appendLog(`${u.name} 踩到小恢复格子：+${u.hp-beforeHp}HP +${u.sp-beforeSp}SP`);
        showGainFloat(u, u.hp-beforeHp, u.sp-beforeSp);
      }
      if(webTraps.has(key)){
        webTraps.delete(key);
        webTrapRevealed.add(key);
        applyStunOrStack(u, 1, {bypass:true, reason:'蛛网陷阱'});
        appendLog(`${u.name} 踩中了蛛网陷阱，被定住！`);
        showStatusFloat(u,'蛛网',{type:'debuff', offsetY:-70});
      }
    }
  }

  // 软肋格：文档要求“任何人踩上都会让 Lirathe 掉下来”，不是只有 Lirathe 自己踩。
  for(const key of keys){
    if(!weakSpots.has(key)) continue;
    weakSpots.delete(key);
    const boss = units['lirathe'];
    if(boss && boss.hp>0){
      boss._highGroundTurns = 0;
      boss._weakSpotVulnTurns = 999;
      boss._weakSpotVulnUntilStunEnds = true;
      updateStatusStacks(boss, 'stunned', Math.max(1, (boss.status.stunned||0)+1), {label:'眩晕', type:'debuff'});
      appendLog(`${u.name} 触发软肋格，${boss.name} 从高处坠落：眩晕+1，减伤降为35%，受到伤害+50%直到眩晕结束。`);
      showStatusFloat(boss,'软肋坠落',{type:'debuff', offsetY:-84});
    }
  }
}
function processSpecialSpawnsAtEnemyTurnStart(){
  if(specialSpawnsDisabled || lirathePhase !== 2 || finalSequenceTriggered) return;
  if(roundsPassed >= nextBudSpawnTurn){
    spawnConsciousnessBud();
    nextBudSpawnTurn += 5;
  }
  if(roundsPassed >= nextRecoveryTileSpawnTurn){
    spawnRecoveryTile();
    nextRecoveryTileSpawnTurn += 10;
  }
  const boss = units['lirathe'];
  if(boss && boss.hp>0 && boss.passives.includes('climb') && !boss.status.stunned){
    const nearWall = boss.r <= 1 || boss.c <= 1 || boss.r + (boss.size||1) - 1 >= ROWS || boss.c + (boss.size||1) - 1 >= COLS;
    if((nearWall || Math.random()<0.18) && Math.random()<0.35){
      boss._highGroundTurns = 9999;
      appendLog(`${boss.name} 的“攀爬”触发：爬上墙面高处；在软肋/SP崩溃/终局剧情前无法被直接攻击。`);
      showStatusFloat(boss,'高处',{type:'buff', offsetY:-80});
    }
  }
}
function processConsciousnessBudRecovery(){
  for(const u of Object.values(units)){
    if(!u || u.hp<=0 || !u.id.startsWith('consciousness_bud_')) continue;
    if(roundsPassed - (u._lastDamagedRound ?? roundsPassed) >= 3){
      const before = u.hp;
      u.hp = Math.min(u.maxHp, u.hp + 20);
      if(u.hp > before){
        appendLog(`${u.name} 三回合未受击：恢复 ${u.hp-before} HP`);
...
async function consciousnessBudThrust(u, dir){
  const area = range_forward_n(u, 3, dir||u.facing);
  if(area.length===0){ unitActed(u); return; }
  await telegraphThenImpact(area);
  for(const cell of area){
    const target = getUnitAt(cell.r, cell.c);
    if(target && target.side !== u.side){
      damageUnit(target.id, 15, 5, `${u.name} 花苞刺击`, u.id, {skillFx:'lirathe:刺斩'});
    }
  }
  unitActed(u);
}
function cleanupSpecialTilesAndBuds(){
  weakSpots.clear();
  recoveryTiles.clear();
  webTraps.clear();
  webTrapRevealed.clear();
  for(const id of Object.keys(units)){
    if(id.startsWith('consciousness_bud_')) delete units[id];
  }
  appendLog('终局：清除意识花苞、软肋、恢复格与蛛网陷阱。');
}
function findLiratheFinalPosition(boss, karma){
  const raw = [
    {r:karma.r-1,c:karma.c-2,dir:'right'},
    {r:karma.r-1,c:karma.c+1,dir:'left'},
    {r:karma.r-2,c:karma.c-1,dir:'down'},
    {r:karma.r+1,c:karma.c-1,dir:'up'},
    {r:karma.r,c:karma.c-2,dir:'right'},
    {r:karma.r,c:karma.c+1,dir:'left'},
  ];
  for(const p of raw){
    const r = clampValue(p.r,1,ROWS-1), c = clampValue(p.c,1,COLS-1);
    if(canPlace2x2(boss,r,c)) return {r,c,dir:p.dir};
  }
  return {r:clampValue(karma.r-1,1,ROWS-1), c:clampValue(karma.c-2,1,COLS-1), dir:'right'};
}
function showStoryLines(lines){
  return new Promise(resolve=>{
    ...
    let idx = 0;
    const render = ()=>{
      text.innerHTML = String(lines[idx] || '').replace(/\n/g,'<br>');
      btn.textContent = idx >= lines.length-1 ? '结束' : '继续';
    };
    btn.onclick = ()=>{
      idx += 1;
      if(idx >= lines.length){
...
async function runLiratheFinalSequence(){
  const boss = units['lirathe'];
  const karma = units['karma'];
  if(!boss || !karma || boss.hp<=0 || karma.hp<=0) return;
  setInteractionLocked(true);
  currentSide = 'enemy';
  playerSteps = 0; enemySteps = 0; updateStepsUI();
  await sleep(3000);
  const pos = findLiratheFinalPosition(boss, karma);
  boss.r = pos.r; boss.c = pos.c; setUnitFacing(boss, pos.dir);
  appendLog(`${boss.name} 瞬间贴近 Karma。`);
  cameraFocusOnCell(karma.r, karma.c, {scale:1.35});
  renderAll();
  await sleep(500);
  const area = forwardRect2x2(boss, boss.facing, 2, 2);
  for(let i=1;i<=7;i++){
    await telegraphThenImpact(area.length ? area : [{r:karma.r,c:karma.c}]);
    damageUnit(karma.id, 15, 5, `${boss.name} 掏心掏肺·终局 ${i}/7`, boss.id, {lockTargetAtOne:true, ignoreLiratheRage:true});
    corrosionStacks.set(karma.id, (corrosionStacks.get(karma.id)||0) + 1);
    if(karma.hp <= 1){ karma.hp = 1; }
    renderAll();
    await sleep(220);
  }
  karma._lockedAtOne = true;
  karma.hp = Math.max(1, karma.hp);
  await showStoryLines([
    '（Lirathe 即将杀掉 Karma 时，看到了周围血坑中的倒影）',
    '（和女孩子没有任何关系的自己）',
    'Lirathe：（我在干什么。。。）',
    'Lirathe：（我为什么会变成这样）',
    'Lirathe：（。。。。）',
  ...
  boss._permanentStun = true;
  boss._highGroundTurns = 0;
  boss._weakSpotVulnTurns = 0;
  updateStatusStacks(boss, 'stunned', 999, {label:'永久眩晕', type:'debuff'});
  appendLog(`${boss.name} 停止行动，进入永久眩晕。`);
  setInteractionLocked(false);
  currentSide = 'player';
...
function buildSkillFactoriesForUnit(u){
  ...
  if(u.id==='adora'){
    ...
    );  } else if(u.id && u.id.startsWith('consciousness_bud_')){
    F.push({ key:'花苞刺击', prob:1.0, cond:()=>true, make:()=> skill('花苞刺击',1,'red','前方3格 15HP+5SP',
      (uu,aimDir)=> aimDir? range_forward_n(uu,3,aimDir) : (()=>{const a=[]; for(const d in DIRS) range_forward_n(uu,3,d).forEach(x=>a.push(x)); return a;})(),
      async (uu,desc)=> { const dir = desc && desc.dir ? desc.dir : uu.facing; await consciousnessBudThrust(uu, dir); },
      {},
      {castMs:800}
    )});
    ...
    // Lirathe Phase 2 Skills (Transformed)
      ...
      F.push(
        ...
        { key:'你在哪', prob:0.30, cond:()=>true, make:()=> skill('你在哪',2,'red','6x6 AOE 10SP+腐蚀+看见',
          (uu)=> range_footprint_square(uu,2).map(c=>({...c,dir:uu.facing})),
...
function buildGrid(){
  ...
  for(let r=1;r<=ROWS;r++){
    for(let c=1;c<=COLS;c++){
      ...
      const k = cellKey(r,c);
      if(weakSpots.has(k)) cell.classList.add('weak-spot');
      if(recoveryTiles.has(k)) cell.classList.add('recovery-tile');
      if(webTrapRevealed.has(k)) cell.classList.add('web-trap-revealed');
...
function getSpBarDisplay(u){
  // For units with negative SP (like Lirathe with spFloor=-100):
...
function onCellClick(r,c){
  ...
  // Lirathe restriction: cannot move after casting a skill
  if(sel.id === 'lirathe' && sel._usedSkillThisTurn){ 
    ...
    if(u.side==='enemy'){ skillPool.innerHTML = `<div class="partyRow small">敌方单位（AI 控制），无法操作</div>`; }
      ...
      for(const sk of pool){
        ...
        card.addEventListener('click', ()=>{
          ...
          // Lirathe restriction: cannot cast skill after moving
          if(u.id === 'lirathe' && u.stepsMovedThisTurn > 0){
...
function registerUnitMove(u){
  ...
  if(u.id === 'lirathe' && lirathePhase === 1 && u.passives.includes('swiftAgile') && u.stepsMovedThisTurn >= 3){
    const current = u.status.agileStacks || 0;
    if(current < 3){
      updateStatusStacks(u, 'agileStacks', current + 1, {label:'灵活', type:'buff'});
      appendLog(`${u.name} 的“迅速敏捷”触发：移动累计≥3，获得灵活。`);
    }
  }
  processSpecialTileEntry(u);
}
...
function processUnitsTurnStart(side){
  turnIndex += 1;
  if(side === 'enemy'){
    processSpecialSpawnsAtEnemyTurnStart();
    processConsciousnessBudRecovery();
  }
  for(const id in units){
    const u=units[id];
  ...
  for(const id in units){
    ...
    if(u.id==='lirathe' && side==='enemy' && u.turnsStarted % 5 === 0){
      const before = enemySteps;
      enemySteps = Math.max(0, enemySteps - 2);
      if(before !== enemySteps){ appendLog('疲劳的躯体：Lirathe 本回合额外 -2 步'); updateStepsUI(); }
    }
    ...
    if(u.status.resentStacks && u.status.resentStacks>0){
      ...
      applySpDamage(u, loss, {reason:`${u.name} 被怨念侵蚀：SP -{delta}`});
    ...
    if(corrosion > 0){
      ...
      damageUnit(u.id, corrosionDmg, 0, `${u.name} 受腐蚀侵蚀 (-${corrosion}层)`, null, {trueDamage:true});
      corrosionStacks.set(u.id, corrosion - 1);
      if(corrosionStacks.get(u.id) === 0){
        appendLog(`${u.name} 的腐蚀已清除`);
      }
    }
    if(u._weakSpotVulnTurns && u._weakSpotVulnTurns > 0 && u._weakSpotVulnUntilStunEnds && (!u.status || u.status.stunned<=0)){
      u._weakSpotVulnTurns = 0;
      u._weakSpotVulnUntilStunEnds = false;
      appendLog(`${u.name} 的软肋易伤解除。`);
    }
  }
}
function processUnitsTurnEnd(side){
  ...
  for(const id in units){
    ...
    if(u.status.stunned>0){
      if(u._permanentStun){
        updateStatusStacks(u,'stunned', 999, {label:'永久眩晕', type:'debuff'});
      } else {
        const next = Math.max(0, u.status.stunned-1);
        updateStatusStacks(u,'stunned', next, {label:'眩晕', type:'debuff'});
        appendLog(`${u.name} 的眩晕减少 1（剩余 ${u.status.stunned}）`);
        if(u.status.stunned<=0 && u._spCrashVuln){
          u._spCrashVuln = false;
          appendLog(`${u.name} 的 SP 崩溃易伤解除`);
        }
      }
    }
    if(u._highGroundTurns && u._highGroundTurns > 0 && u.id !== 'lirathe'){
      u._highGroundTurns = Math.max(0, u._highGroundTurns - 1);
      if(u._highGroundTurns === 0) appendLog(`${u.name} 从高处回到地面。`);
    }
    if(u._weakSpotVulnTurns && u._weakSpotVulnTurns > 0 && !u._weakSpotVulnUntilStunEnds){
      u._weakSpotVulnTurns = Math.max(0, u._weakSpotVulnTurns - 1);
    }
...
async function exhaustEnemySteps(){
  ...
  while(currentSide==='enemy' && enemySteps>0){
    ...
    for(const en of livingEnemies){
      ...
      if(candidates.length===0 && en.id==='lirathe' && !en._designPenaltyTriggered && typeof en.maxMovePerTurn==='number' && (en.stepsMovedThisTurn||0) >= en.maxMovePerTurn){
        applyLiratheDesignPenalty();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  liratheBGM = document.getElementById('liratheBGM');
  ...
  appendLog('旧情未了：地图 9x26，Lirathe 两阶段 Boss 战。');
  appendLog('Lirathe 需叠满4层眩晕才会进入眩晕状态，SP 崩溃将触发特殊疲劳崩溃。');
  appendLog('二阶段已接入：意识花苞、软肋、恢复格、蛛网陷阱、终局剧情。');
