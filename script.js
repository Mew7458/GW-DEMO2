...
const COIN_SCRATCH_COST = 1;
const POINT_SCRATCH_COST = 5;

// 金币刮刮乐：和积分抽奖分池，不再把金币/积分塞进同一张牌里。
const COIN_SCRATCH_OPTIONS = [
  { type: 'empty', value: 'empty', label: '空', weight: 65, max: SCRATCH_CELL_COUNT },
  { type: 'coins', value: 1, label: '1金币', weight: 40, max: 21 },
  { type: 'coins', value: 2, label: '2金币', weight: 30, max: 18 }
];

// 积分抽奖：专门的积分池；每张积分抽奖至少保证一种积分奖励出现 3 个。
const POINT_SCRATCH_OPTIONS = [
  { type: 'points', value: 1, label: '1积分', weight: 21, max: 21 },
  { type: 'points', value: 15, label: '15积分', weight: 15, max: 15 },
  { type: 'points', value: 20, label: '20积分', weight: 12, max: 12 },
  { type: 'points', value: 25, label: '25积分', weight: 6, max: 6 },
  { type: 'points', value: 30, label: '30积分', weight: 3, max: 3 }
];
...
function shuffleScratchGrid(grid) {
...
function cloneScratchItem(option) {
  return {
    type: option.type,
    value: option.value,
    label: option.label
...
function generateCoinScratchGrid() {
  const counts = new Map();
  const grid = [];

  while (grid.length < SCRATCH_CELL_COUNT) {
    const availableOptions = COIN_SCRATCH_OPTIONS.filter(option => {
      if (option.type === 'empty') return true;
      return (counts.get(`${option.type}:${option.value}`) || 0) < option.max;
    });

    const picked = getWeightedRandom(availableOptions);
    const key = `${picked.type}:${picked.value}`;
    counts.set(key, (counts.get(key) || 0) + 1);
    grid.push(cloneScratchItem(picked));
  }

  return shuffleScratchGrid(grid);
}

function generatePointScratchGrid() {
  const counts = new Map();
  const grid = [];
  const guaranteed = getWeightedRandom(POINT_SCRATCH_OPTIONS);
  const guaranteedKey = `${guaranteed.type}:${guaranteed.value}`;

  for (let i = 0; i < 3; i++) {
    grid.push(cloneScratchItem(guaranteed));
  }
  counts.set(guaranteedKey, 3);

  while (grid.length < SCRATCH_CELL_COUNT) {
    const availableOptions = POINT_SCRATCH_OPTIONS.filter(option => {
      return (counts.get(`${option.type}:${option.value}`) || 0) < option.max;
    });
...
function generateScratchGrid(poolType = 'coins') {
  if (poolType === 'points') {
    return generatePointScratchGrid();
  }
  return generateCoinScratchGrid();
}

function getScratchItemKey(item) {
  if (!item) return 'missing';
  return `${item.type}:${item.value}`;
}

function getScratchItemDisplay(item) {
  if (!item) return '?';
  if (item.type === 'empty') return '空';
  if (item.type === 'coins') return `${item.value}币`;
  if (item.type === 'points') return `${item.value}分`;
  return String(item.value ?? '?');
}

function getScratchRewardName(item) {
  if (!item) return '未知';
  if (item.type === 'empty') return '空';
  if (item.type === 'coins') return `${item.value} 金币`;
  if (item.type === 'points') return `${item.value} 积分`;
  return String(item.label ?? item.value ?? '未知');
}
...
function loadSelectedSkills() {
  ...
  return saved ? JSON.parse(saved) : {
    adora: { green: null, gray: null, blue: null, pink: null, white: null, red: null, purple: null, orange: [] },
    karma: { green: null, gray: null, blue: null, pink: null, white: null, red: null, purple: null, orange: [] },
    dario: { green: null, gray: null, blue: null, pink: null, white: null, red: null, purple: null, orange: [] }
  };
}
...
function createEmptySkillSelection() {
  return { green: null, gray: null, blue: null, pink: null, white: null, red: null, purple: null, orange: [null, null] };
}
...
pushRect(3, 4, 5, 6);
    ...
    enemies: [
      ...
      },
      {
        ...
        skills: [
          ...
          { name: '穿刺狙击（2 步）', detail: '直线 6 格 30 伤害并附流血。出现概率 60%（压迫后停用）。' },
    ...
    enemies: [
      {
        ...
        skills: [
          ...
          { name: '不讲理的蛮力劈砍（1 步）', detail: '对前方 6 格内所有敌方单位造成 45 HP+5 SP 并叠 1 层破伤风。出现概率 75%。' },
          { name: '血浪（2 步）', detail: '对上下左右 4 行（6 格）所有敌方单位造成 35 HP+20 SP；若命中任意单位，以其为中心再次对上下左右 4 行（6 格）造成 35 HP+20 SP。出现概率 60%。' },
          { name: '一股鱼腥味。。（3 步）', detail: '引爆场上所有单位的破伤风层数（每层 18 真实伤害），并为自身回复 5 HP+2 SP。出现概率 40%。' },
          { name: '退潮（5 步）', detail: '对前方扇形（7/5/3 格）所有敌方单位造成 200 HP+50 SP 并叠 5 层破伤风；若命中目标已有 ≥5 层破伤风，则追加一次不讲理的蛮力劈砍；任一段伤害若会将目标打到 50 HP 以下，则锁血在 50。出现概率 10%。' },
        ],
      },
    ],
  },
  ...
  sisterBattle: {
    id: 'sisterBattle',
    name: '西斯特',
    subtitle: '图书馆画布 Boss 战',
    size: '12 × 20',
    narrative: [
      '西斯特会把战场变成画布：颜色状态、颜料印记和二阶段技能池都会滚雪球。',
      '她在 HP 200 以下会进入“意识到了”，关闭原技能池，改用黑色与发布机制压制玩家。',
    ],
    brief: [
      '地图 12×20，三人组在左侧，西斯特位于右侧。',
      '西斯特：Boss / 等级45 / HP550 / SP125 / 护甲15。',
      '核心机制：红蓝黄橙绿紫黑七种颜色状态、颜料、灵感、完成的作品。',
      'HP≤200 后进入二阶段，SP 永远低于0，并启用发泄性作画与发布。',
    ],
    map: {
      rows: 12,
      cols: 20,
      ...
      cover: [],
      players: [
        { row: 6, col: 2, label: 'Ad', type: 'player', tone: 'adora' },
        { row: 4, col: 2, label: 'Da', type: 'player', tone: 'dario' },
        { row: 8, col: 2, label: 'Ka', type: 'player', tone: 'karma' },
      ],
      enemies: [
        { row: 6, col: 17, label: '西', type: 'boss' },
      ],
    },
    enemies: [
      {
        ...
        icon: '🎨',
        rank: 'Boss / 等级45 / 占1格',
        summary: 'HP 550 · SP 125（降至 -45：20真实自伤、失控1回合并-1步，随后一阶段恢复75；二阶段维持负SP）· 护甲15',
        threat: 'boss',
        skills: [
          { name: '特殊颜色状态', detail: '红/绿/紫在受伤时触发，蓝/黑在回合开始触发，黄/橙在造成伤害时触发；红蓝黄可混合出紫橙绿，紫橙绿可混出黑。' },
          { name: '被动：如此锋利', detail: '西斯特每次造成伤害都会叠1层流血。' },
          { name: '被动：布网 / 灵感', detail: 'HP>200时，每回合获得颜料并+5SP；颜料>25时每回合获得灵感。' },
          { name: '随性作画（1步）', detail: '消耗颜料，随机释放红怨/蓝调/黄堕，添加红、蓝、黄颜色状态。出现概率80%。' },
          { name: '剧本创作（3步）', detail: '5×5内选敌，瞬移到目标面前，推开目标周围其他敌人，造成5HP并叠流血。出现概率60%。' },
          { name: '这里是我的画布（3步）', detail: '传送到随机敌方单位周围5×5格子。出现概率20%。' },
          { name: '签名（全部步数）', detail: '当任意单位拥有除黑色外全部颜色时出现：补色、连斩、25真实伤害、添加黑色，并给自身完成的作品。' },
          { name: '意识到了（二阶段）', detail: 'HP≤200时关闭原技能池，所有颜色转黑，SP永远在0以下；到-35时获得90%SP防御直到-45崩溃。' },
          { name: '发泄性作画（2步）', detail: '二阶段消耗颜料，随机释放怨/悲/奈，围绕黑色、流血引爆、恢复与增伤展开。出现概率80%。' },
          { name: '发布（全部步数）', detail: '累计黑色足够后唯一可用：拖入画布、连续攻击、引爆黑色层数并随机眩晕。' },
        ],
      },
    ],
  },
...
function initStageBoard() {
  ...
  if (enterBtn) {
    enterBtn.addEventListener('click', () => {
      ...
      if (currentStageId === 'sisterBattle') {
        window.location.href = 'sister-boss-battle.html';
        return;
      }
...
const skillLibrary = {
  ...
  karma: [
    ...
    { id: 'karma_charge', name: '蓄力', color: 'orange', cost: '2步', description: '给自己上一层鸡血并进入蓄力状态（本回合无法移动或使用技能）。下回合开始消耗1层蓄力，并必定获得额外技能“爆”（技能池一次性只能有一个）。', probability: '30%', minLevel: 50 },
...
function pruneSelectedSkillsForStage(stageId, characterId, availableSkills) {
  ...
  let changed = false;
  ['green', 'gray', 'blue', 'pink', 'white', 'red', 'purple'].forEach((color) => {
...
function renderScratchSection(container) {
  ...
  scratchSection.innerHTML = `
    <div class="scratch-header">
      <h4>抽奖系统</h4>
      <p class="scratch-hint">金币刮刮乐和积分抽奖是两个不同池子；金币刮刮乐每次消耗 1 币，积分抽奖每次消耗 5 币。7 × 5 格，翻到三个相同内容后立即结算并结束本局。</p>
    </div>
    <div class="scratch-pool-info">
      <div><b>金币池：</b>每次消耗 1 币；结果只会出现空、1金币、2金币。</div>
      <div><b>积分池：</b>每次消耗 5 币；结果只会出现 1 / 15 / 20 / 25 / 30 积分，并保证每张至少有一种积分奖励出现 3 个。</div>
    </div>
  `;
  ...
  const startCoinScratchBtn = document.createElement('button');
  startCoinScratchBtn.className = 'scratch-start-btn scratch-coin-btn';
  startCoinScratchBtn.textContent = `开始金币刮刮乐（${COIN_SCRATCH_COST}币）`;
  scratchControls.appendChild(startCoinScratchBtn);

  const startPointScratchBtn = document.createElement('button');
  startPointScratchBtn.className = 'scratch-start-btn scratch-point-btn';
  startPointScratchBtn.textContent = `开始积分抽奖（${POINT_SCRATCH_COST}币）`;
  scratchControls.appendChild(startPointScratchBtn);
  ...
  scratchStatus.textContent = `选择金币刮刮乐或积分抽奖开始。金币刮刮乐 ${COIN_SCRATCH_COST} 币一次，积分抽奖 ${POINT_SCRATCH_COST} 币一次。`;
  ...
  function updateScratchButtons() {
    // 金币不足时仍允许点击，让按钮能给出“硬币不足”的提示；只有抽奖进行中才锁按钮。
    startCoinScratchBtn.disabled = scratchState.active;
    startPointScratchBtn.disabled = scratchState.active;
    exchangeBtn.disabled = scratchState.active || loadPoints() < 20;
  }
  ...
  function renderScratchGrid() {
    ...
    scratchState.grid.forEach((item, index) => {
      ...
      cell.dataset.pool = scratchState.poolType || '';
      scratchGrid.appendChild(cell);
    });
  }

  function startScratch(poolType) {
    const cost = poolType === 'points' ? POINT_SCRATCH_COST : COIN_SCRATCH_COST;
    const currentCoins = loadCoins();
    const poolName = poolType === 'points' ? '积分抽奖' : '金币刮刮乐';
    ...
    if (currentCoins < cost) {
      scratchStatus.textContent = `${poolName}需要 ${cost} 币，当前只有 ${currentCoins} 币。`;
      showToast(`${poolName}硬币不足：需要 ${cost} 币，当前 ${currentCoins} 币`);
      updateScratchButtons();
      return;
    }

    saveCoins(currentCoins - cost);
    scratchState.poolType = poolType;
    scratchState.grid = generateScratchGrid(poolType);
    scratchState.active = true;
    scratchStatus.textContent = `${poolName}进行中：已消耗 ${cost} 币。翻出三个相同内容即可获得奖励。`;
    renderScratchGrid();
    updateCurrencyDisplay();
  }

  function revealScratchCell(target, item) {
    target.classList.add('revealed');
    target.textContent = getScratchItemDisplay(item);
    if (item.type === 'empty') {
      target.classList.add('scratch-empty');
    } else if (item.type === 'coins') {
      target.classList.add('scratch-coins');
  ...
  scratchGrid.addEventListener('click', (event) => {
    ...
    revealScratchCell(target, item);

    const winningKey = getScratchItemKey(item);
    scratchState.counts.set(winningKey, (scratchState.counts.get(winningKey) || 0) + 1);
    ...
    if (count >= 3) {
      if (item.type === 'empty') {
        ...
        showToast(`金币刮刮乐奖励：获得 ${item.value} 金币（总计 ${newCoins} 金币）`);
      } else if (item.type === 'points') {
        const newPoints = addPoints(item.value);
        updateCurrencyDisplay();
        endScratchRound(`本局结束：获得 ${item.value} 积分！`);
        showToast(`积分抽奖奖励：获得 ${item.value} 积分（总计 ${newPoints} 积分）`);
      } else {
        endScratchRound(`本局结束：获得 ${getScratchRewardName(item)}。`);
      }
    }
  });

  startCoinScratchBtn.addEventListener('click', () => startScratch('coins'));
  startPointScratchBtn.addEventListener('click', () => startScratch('points'));
    ...
    sections: [
      {
        heading: '持续状态',
        bullets: [
          '流血：每回合消耗 1 层，可叠加；每层流血强度增加 2HP 的流血伤害。',
